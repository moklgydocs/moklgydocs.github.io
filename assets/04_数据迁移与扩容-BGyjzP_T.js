import{A as e,E as t,d as n,l as r,p as i}from"./runtime-core.esm-bundler-tCF-J6l5.js";import{t as a}from"./app-X6fsa0X9.js";var o=JSON.parse(`{"path":"/%E5%90%8E%E7%AB%AF%E5%BC%80%E5%8F%91/Redis/09_%E7%94%9F%E4%BA%A7%E7%BA%A7%E6%9E%B6%E6%9E%84%E8%AE%BE%E8%AE%A1/04_%E6%95%B0%E6%8D%AE%E8%BF%81%E7%A7%BB%E4%B8%8E%E6%89%A9%E5%AE%B9.html","title":"数据迁移与扩容","lang":"zh-CN","frontmatter":{"title":"数据迁移与扩容","icon":"fa6-solid:arrows-left-right","order":2,"category":["Redis"],"tag":["扩容","迁移","槽迁移","redis-shake","双写","预分片","缩容"]},"git":{"createdTime":1780588404000,"updatedTime":1780588404000,"contributors":[{"name":"jackie.liu","username":"","email":"moklgy@foxmail.com","commits":1}]},"readingTime":{"minutes":17.17,"words":5151},"filePathRelative":"后端开发/Redis/09_生产级架构设计/04_数据迁移与扩容.md"}`),s={name:`04_数据迁移与扩容.md`};function c(a,o,s,c,l,u){let d=e(`Mermaid`);return t(),r(`div`,null,[o[0]||=n(`<h1 id="数据迁移与扩容" tabindex="-1"><a class="header-anchor" href="#数据迁移与扩容"><span>数据迁移与扩容</span></a></h1><blockquote><p>Redis 扩容和迁移是生产环境中最具挑战性的运维操作之一。内存不足、QPS 瓶颈、业务增长都要求 Redis 具备在线扩容能力，但扩容过程中的数据一致性、服务可用性和回滚预案都需要精心设计。本文将从扩容场景分析出发，深入各种扩容方案、迁移工具和最佳实践。</p></blockquote><h2 id="_1-扩容场景分析" tabindex="-1"><a class="header-anchor" href="#_1-扩容场景分析"><span>1. 扩容场景分析</span></a></h2><h3 id="_1-1-何时需要扩容" tabindex="-1"><a class="header-anchor" href="#_1-1-何时需要扩容"><span>1.1 何时需要扩容</span></a></h3><div class="language-text line-numbers-mode" data-highlighter="shiki" data-ext="text" style="background-color:#282c34;color:#abb2bf;"><pre class="shiki one-dark-pro vp-code"><code class="language-text"><span class="line"><span>Redis 扩容触发条件：</span></span>
...
<span class="line"><span>└───────────────────────────────────────────────────┘</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2-3-水平扩容方案" tabindex="-1"><a class="header-anchor" href="#_2-3-水平扩容方案"><span>2.3 水平扩容方案</span></a></h3>`,13),i(d,{code:``}),o[1]||=n(`<h2 id="_3-主从迁移" tabindex="-1"><a class="header-anchor" href="#_3-主从迁移"><span>3. 主从迁移</span></a></h2><h3 id="_3-1-主从-→-sentinel-迁移" tabindex="-1"><a class="header-anchor" href="#_3-1-主从-→-sentinel-迁移"><span>3.1 主从 → Sentinel 迁移</span></a></h3><div class="language-text line-numbers-mode" data-highlighter="shiki" data-ext="text" style="background-color:#282c34;color:#abb2bf;"><pre class="shiki one-dark-pro vp-code"><code class="language-text"><span class="line"><span>主从 → Sentinel 迁移步骤：</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>└──────────┘ └──────────┘ └──────────┘ └──────────┘</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_4-2-迁移槽位" tabindex="-1"><a class="header-anchor" href="#_4-2-迁移槽位"><span>4.2 迁移槽位</span></a></h3>`,9),i(d,{code:``}),o[2]||=n(`<div class="language-text line-numbers-mode" data-highlighter="shiki" data-ext="text" style="background-color:#282c34;color:#abb2bf;"><pre class="shiki one-dark-pro vp-code"><code class="language-text"><span class="line"><span>槽迁移详细流程：</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>  └──────────────────────────────────────────────────┘</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_9-双写方案" tabindex="-1"><a class="header-anchor" href="#_9-双写方案"><span>9. 双写方案</span></a></h2><h3 id="_9-1-双写架构" tabindex="-1"><a class="header-anchor" href="#_9-1-双写架构"><span>9.1 双写架构</span></a></h3>`,29),i(d,{code:``}),o[3]||=n(`<h3 id="_9-2-c-双写实现" tabindex="-1"><a class="header-anchor" href="#_9-2-c-双写实现"><span>9.2 C# 双写实现</span></a></h3><div class="language-csharp line-numbers-mode" data-highlighter="shiki" data-ext="csharp" style="background-color:#282c34;color:#abb2bf;"><pre class="shiki one-dark-pro vp-code"><code class="language-csharp"><span class="line"><span style="color:#7F848E;font-style:italic;">// 双写中间件</span></span>
