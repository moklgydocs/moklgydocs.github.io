import{M as e,O as t,c as n,d as r,h as i,m as a,p as o}from"./runtime-core.esm-bundler-jC72uHyJ.js";import{t as s}from"./app-CQDU0Gm9.js";var c=JSON.parse(`{"path":"/%E8%AE%A1%E7%AE%97%E6%9C%BA%E5%AD%A6%E7%A7%91/%E6%B1%87%E7%BC%96%E8%AF%AD%E8%A8%80/03_%E8%BF%87%E7%A8%8B%E4%B8%8E%E6%A0%88/01_%E6%A0%88%E5%B8%A7%E4%B8%8E%E8%BF%87%E7%A8%8B%E8%B0%83%E7%94%A8.html","title":"栈帧与过程调用","lang":"zh-CN","frontmatter":{"title":"栈帧与过程调用","icon":"fa6-solid:layer-group","order":1,"category":["计算机学科","汇编语言"],"tag":["栈"]},"git":{"createdTime":1780564101000,"updatedTime":1780564101000,"contributors":[{"name":"jackie.liu","username":"","email":"moklgy@foxmail.com","commits":1}]},"readingTime":{"minutes":6.65,"words":1994},"filePathRelative":"计算机学科/汇编语言/03_过程与栈/01_栈帧与过程调用.md"}`),l={name:`01_栈帧与过程调用.md`};function u(s,c,l,u,d,f){let p=e(`Mermaid`);return t(),r(`div`,null,[c[0]||=o(`<h1 id="栈帧与过程调用" tabindex="-1"><a class="header-anchor" href="#栈帧与过程调用"><span>栈帧与过程调用</span></a></h1><h2 id="函数调用的底层真相" tabindex="-1"><a class="header-anchor" href="#函数调用的底层真相"><span>函数调用的底层真相</span></a></h2><p>在 C 语言中写 <code>result = add(3, 5)</code> 只是一行代码，但在 CPU 层面，这行代码引发了一连串精密的操作：保存当前执行位置、传递参数、跳转、执行、返回结果、恢复现场。这一切都依赖<strong>栈</strong>和<strong>栈帧</strong>机制。</p><p>如果把函数调用比作寄信，栈帧就是信封——上面写着寄信人地址（返回地址）、信件内容（参数和局部变量），确保信件能准确送达和回复。</p>`,4),i(p,{code:``}),c[1]||=n(`h2`,{id:`栈的工作原理`,tabindex:`-1`},[n(`a`,{class:`header-anchor`,href:`#栈的工作原理`},[n(`span`,null,`栈的工作原理`)])],-1),c[2]||=n(`h3`,{id:`栈的生长方向`,tabindex:`-1`},[n(`a`,{class:`header-anchor`,href:`#栈的生长方向`},[n(`span`,null,`栈的生长方向`)])],-1),c[3]||=n(`p`,null,[a(`x86 的栈向`),n(`strong`,null,`低地址方向增长`),a(`：`)],-1),i(p,{code:``}),c[4]||=o(`<div class="hint-container important"><p class="hint-container-title">栈的双指针</p><ul><li><strong>ESP（栈指针）</strong>：始终指向栈顶（最后压入的数据），随 PUSH/POP 自动移动</li><li><strong>EBP（帧指针）</strong>：指向当前栈帧的固定基准点，函数执行期间不变</li></ul></div><h3 id="为什么需要-ebp" tabindex="-1"><a class="header-anchor" href="#为什么需要-ebp"><span>为什么需要 EBP？</span></a></h3><p>ESP 在函数执行过程中会不断变化（PUSH/POP），而参数和局部变量需要稳定的访问基准。EBP 就是这个&quot;锚点&quot;：</p><ul><li>参数通过 <code>[EBP + 8]</code>、<code>[EBP + 12]</code> 等正偏移访问</li><li>局部变量通过 <code>[EBP - 4]</code>、<code>[EBP - 8]</code> 等负偏移访问</li></ul><h2 id="call-和-ret-指令" tabindex="-1"><a class="header-anchor" href="#call-和-ret-指令"><span>CALL 和 RET 指令</span></a></h2><h3 id="call-调用函数" tabindex="-1"><a class="header-anchor" href="#call-调用函数"><span>CALL - 调用函数</span></a></h3><div class="language-nasm line-numbers-mode" data-highlighter="shiki" data-ext="nasm" style="background-color:#282c34;color:#abb2bf;"><pre class="shiki one-dark-pro vp-code"><code class="language-nasm"><span class="line"><span>call my_function     ; 等价于: push eip_next; jmp my_function</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><p>CALL 做两件事：</p><ol><li>将下一条指令的地址（返回地址）压入栈中</li><li>跳转到目标函数</li></ol><h3 id="ret-返回调用者" tabindex="-1"><a class="header-anchor" href="#ret-返回调用者"><span>RET - 返回调用者</span></a></h3><div class="language-nasm line-numbers-mode" data-highlighter="shiki" data-ext="nasm" style="background-color:#282c34;color:#abb2bf;"><pre class="shiki one-dark-pro vp-code"><code class="language-nasm"><span class="line"><span>ret                  ; 等价于: pop eip; jmp eip</span></span>
...
<span class="line"><span>    ret</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div></div></div><p>同理，ENTER 指令可以替代序言，但现代编译器不使用 ENTER（太慢），通常用 <code>push ebp / mov ebp, esp / sub esp, N</code>。</p><h2 id="小结" tabindex="-1"><a class="header-anchor" href="#小结"><span>小结</span></a></h2>`,13),i(p,{code:``}),c[7]||=o(`<div class="hint-container tip"><p class="hint-container-title">面试要点</p><ul><li>函数调用标准流程：序言（push ebp / mov ebp,esp / sub esp,N）→ 函数体 → 结语（leave / ret）</li><li>参数通过 [EBP+8]、[EBP+12] 访问，局部变量通过 [EBP-N] 访问</li><li>cdecl 由调用者清理栈（add esp,N），stdcall 由被调用者清理（ret N）</li><li>CALL = PUSH 返回地址 + JMP，RET = POP 返回地址 + JMP</li><li>调用者保存 EAX/ECX/EDX，被调用者保存 EBX/ESI/EDI/EBP</li><li>LEAVE = MOV ESP,EBP + POP EBP</li></ul></div><div class="hint-container info"><p class="hint-container-title">原著参考</p><p>本章内容参考自《汇编语言：基于Linux环境（第3版）》Jeff Duntemann 第10章&quot;Decking Out the Procedures&quot;。书中详细讲解了过程调用的完整流程和栈帧管理技术。</p></div>`,2)])}var d=s(l,[[`render`,u]]);export{c as _pageData,d as default};