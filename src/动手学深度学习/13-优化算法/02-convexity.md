> 本文转载自[《动手学深度学习》](https://zh.d2l.ai/)(作者:Aston Zhang、Zachary C. Lipton、李沐、Alexander J. Smola),原文:<https://zh.d2l.ai/chapter_optimization/convexity.html>。著作权归原作者所有,原书采用 CC BY-SA 4.0 许可,本站仅供个人学习使用。
...
这听起来有点抽象，那我们来看一下 下图里的例子。
第一组存在不包含在集合内部的线段，所以该集合是非凸的，而另外两组则没有这样的问题。

![第一组是非凸的，另外两组是凸的。](/d2l/img/pacman.svg)
...
![两个凸集的交集是凸的。](/d2l/img/convex-intersect.svg)
...
在 下图中连接$a$和$b$的线段需要包含一部分既不在$\mathcal{X}$也不在$\mathcal{Y}$中。
因此线段也不在$\mathcal{X} \cup \mathcal{Y}$中，因此证明了凸集的并集不一定是凸的，即*非凸*（nonconvex）的。

![两个凸集的并集不一定是凸的。](/d2l/img/nonconvex.svg)
...
事实上，我们一直在使用这个技巧。
比如权重衰减 相关章节，在目标函数中加入$\frac{\lambda}{2} |\mathbf{w}|^2$，以确保$\mathbf{w}$不会增长太大。
...
同样，我们之前也遇到过，例如在 相关章节中处理梯度截断时，我们通过
...
![Convex Projections.](/d2l/img/projections.svg)

投影的数学定义听起来可能有点抽象，为了解释得更清楚一些，请看 下图。
图中有两个凸集，一个圆和一个菱形。
