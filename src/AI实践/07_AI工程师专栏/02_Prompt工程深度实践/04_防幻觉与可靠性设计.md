...
def validate_response(response: str, retrieved_docs: list[str]) -> dict:
    ...
    # 1. 检查是否包含"无法回答"标记（好的信号）
    if "无法" in response or "未提及" in response:
    ...
    for num in numbers:
        in_source = any(num in doc for doc in retrieved_docs)
        if not in_source and len(num) >= 3:  # 忽略太短的数字
            issues.append(f"数字 {num} 未在参考资料中出现")
