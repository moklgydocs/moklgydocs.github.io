...
class ChineseBM25Retriever:
    def __init__(self, documents: list[Document], k: int = 10):
        self.k = k
        # 使用索引存储原始文档，避免 processed text 做 key 导致碰撞
        self.doc_store = {}  # id -> original doc
        processed_docs = []
    ...
    def invoke(self, query: str) -> list[Document]:
        ...
        # 还原原始文本
        for r in results:
            for idx, orig in self.doc_store.items():
                if chinese_preprocess(orig.page_content) == r.page_content:
                    r.page_content = orig.page_content
                    break
        return results
```
...
中文 RAG 的检索质量很大程度上取决于 Embedding 模型对中文的编码能力。

| 模型 | 维度 | 中文能力 | 多语言 | 推荐场景 |
|------|------|---------|--------|---------|
| BAAI/bge-m3 | 1024 | 强 | 支持 | 通用中英文混合 |
| BAAI/bge-large-zh-v1.5 | 1024 | 极强 | 仅中文 | 纯中文场景 |
| shibing624/text2vec-base-chinese | 768 | 中 | 仅中文 | 轻量级/快速原型 |
| maidalun1020/bce-embedding-base_v1 | 768 | 强 | 支持 | 中文电商/搜索 |
| Alibaba-NLP/gte-Qwen2-7B-instruct | 3584 | 极强 | 支持 | 高精度/有GPU |

**选型建议**：
- 纯中文文档：`bge-large-zh-v1.5`（中文专用，效果最好）
- 中英混合：`bge-m3`（多语言通用）
- 高精度需求：`gte-Qwen2-7B-instruct`（需 GPU）
- 轻量级：`text2vec-base-chinese`（CPU 可跑）
...
class AdaptiveHybridRetriever:
    ...
    def _classify_query(self, query: str) -> str:
        ...
        has_technical_terms = bool(re.search(
            r'[A-Z]{2,}|[a-z]+_[a-z]+|\d{4,}', query
        ))
        is_short = len(query) < 15
        has_keywords = len(query.split()) <= 3  # keyword-style query

        if (is_short or has_keywords) and has_technical_terms:
            return "sparse_heavy"  # BM25 权重 0.7
        elif is_short or has_keywords:
            return "balanced"       # 0.5/0.5
        else:
            return "dense_heavy"    # Dense 权重 0.7
