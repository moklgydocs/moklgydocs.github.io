...
class SQLValidator:
    ...
    def auto_correct(self, sql: str, issues: list[dict]) -> str:
        ...
        for issue in issues:
            ...
            elif issue["type"] == "trailing_semicolon":
                corrected = corrected.rstrip(";")
...
async def execute_query(request: QueryRequest, req: Request):
    ...
    # 4. 执行SQL
    exec_result = engine.execute(gen_result.sql, database_id=request.database_id)
...
class ExecutionEngine:
    ...
    def execute(
        ...
        """执行SQL查询"""
        config = self.connections.get(database_id)
        if not config:
        ...
        try:
            conn = psycopg2.connect(
                ...
                options="-c default_transaction_read_only=on" if config.get("readonly") else "",
            )
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if cursor.description:
                    columns = [
                        {"name": desc[0], "type": self._map_type(desc[1])}
    ...
    @staticmethod
    def _map_type(oid: int) -> str:
        """映射PostgreSQL OID到类型名称"""
        type_map = {
            23: "integer",
            20: "bigint",
            700: "float",
