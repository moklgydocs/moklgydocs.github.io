...
> ⚠️ 以下示例省略了认证中间件，生产环境必须添加。
> 推荐方案：JWT Token + RBAC 权限控制 + 多租户数据隔离。
> 详见下方"认证与权限"章节。
...
生产环境的知识库必须实现认证和权限控制，确保不同租户只能访问自己的数据。
...
async def query(
    ...
    user: dict = Depends(verify_token),  # ← 添加认证
):
    # user["tenant_id"] 可用于多租户数据隔离
    result = await rag_service.query(request.question, tenant_id=user.get("tenant_id"))
    return result
```
...
使用 RAGAS 框架对 RAG 系统进行自动化评估，量化检索和生成质量。
...
class RAGEvaluator:
    ...
    async def evaluate(self, test_data: list[dict]) -> dict:
        """
        Args:
            test_data: [{"question": ..., "ground_truth": ..., "context": ...}]
        """
        ...
        for item in test_data:
            # 运行 RAG 获取结果
            result = await self.rag.query(item["question"])
