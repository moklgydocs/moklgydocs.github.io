...
class ConversationMemory:
    ...
    def add_message(self, session_id: str, role: str, content: str):
        ...
        # ⚠️ 当前 Redis TTL=3600s，Redis 重启会丢失所有会话
        # 生产环境应添加：
        # 1. Redis RDB/AOF 持久化
        # 2. 重要会话同步写入数据库（PostgreSQL/MySQL）
        # 3. Redis 不可用时降级到数据库查询
...
8. **速率限制**：防止恶意用户耗尽 API 额度

```python
# ⚠️ 生产环境需添加速率限制，防止恶意用户耗尽 API 额度
# 推荐方案：
# from slowapi import Limiter
# limiter = Limiter(key_func=get_remote_address)
# @app.post("/api/chat")
# @limiter.limit("20/minute")  # 每个 IP 每分钟最多 20 次
```
