...
对于多商品多仓库的库存优化问题，可以使用数学规划求解全局最优解。

**注：本模型包含概率约束(P(D_ij <= I_ij + Q_ij) >= alpha_i)和二元决策变量delta_ij，实际为随机混合整数规划问题(MILP with chance constraints)，需用分支定界法+场景法求解，不能直接用标准LP求解器。**

```
目标: min Σ_i Σ_j (持有成本_ij * I_ij + 缺货成本_ij * B_ij + 订货成本_ij * δ_ij)

约束条件:
1. 库存平衡: I_ij(t+1) = max(0, I_ij(t) + Q_ij(t) - D_ij(t))
   缺货量:   B_ij(t) = max(0, D_ij(t) - I_ij(t) - Q_ij(t))
...
报童模型是单周期库存决策的基础模型，适用于季节性商品、生鲜、时尚品等不可补货场景。

**核心问题**：订货量Q，需求D为随机变量
- 订多了 → 残值损失：成本C_u = 售价 - 进价
- 订少了 → 缺货损失：成本C_o = 进价 - 残值

**最优订货量**：使缺货概率等于临界比

$$P(D > Q^*) = \frac{C_o}{C_u + C_o}$$
...
def newsvendor_order(
    ...
    cu = price - cost  # 缺货成本（少赚的利润）
    co = cost - salvage  # 残值成本（多订的损失）
    critical_ratio = cu / (cu + co)

    optimal_q = int(norm.ppf(critical_ratio, loc=mean_demand, scale=std_demand))
    return max(0, optimal_q)
```

**适用场景**：大促备货、生鲜日配、季节性服装、节日礼品
