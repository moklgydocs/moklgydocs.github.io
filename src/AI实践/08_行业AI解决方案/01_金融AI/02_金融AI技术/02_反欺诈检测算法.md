...
| 欺诈模式 | 描述 | 检测要点 |
|----------|------|----------|
| 羊毛党 | 有组织地批量注册账户，利用促销/优惠券/返现活动套利 | 批量注册检测、设备指纹聚合、IP聚集、行为模式异常 |
| 猫池 | 使用猫池设备同时持有数百张SIM卡，批量接收验证码 | 单IP多账号、验证码请求频率异常、设备ID聚类 |
| 撞库 | 利用已泄露的用户名密码组合，批量尝试登录 | 登录失败率突增、多账号同一IP/设备、凭证来源暗网匹配 |
| 套现 | 利用信用卡虚假交易将信用额度转为现金 | 商户异常交易模式、大额整数交易、买卖方关联 |
| 账户接管(ATO) | 通过钓鱼/社工/撞库获取账户控制权 | 登录地变化、设备变更、密码修改后快速转账 |
| 设备伪造 | 使用模拟器/改机工具伪造设备信息 | Root/Jailbreak检测、模拟器指纹、传感器数据异常 |
| 代理IP | 使用代理/VPN隐藏真实IP，绕过地域限制 | IP信誉库匹配、WebRTC泄漏检测、时区不一致 |
| 虚假商户 | 注册虚假商户进行洗钱或套现 | 交易集中度异常、商户与客户关联、经营规模与流水不匹配 |
```
