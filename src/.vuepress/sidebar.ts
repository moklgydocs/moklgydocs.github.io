import { sidebar } from "vuepress-theme-hope";

export default sidebar({
  "/": [
    {
      text: "后端开发",
      icon: "fa6-solid:code",
      prefix: "后端开发/",
      collapsible: true,
      children: [
        {
          text: "CSharp",
          icon: "fa6-brands:microsoft",
          prefix: "CSharp/",
          collapsible: true,
          children: [
            "",
            "垃圾回收机制",
            "委托",
            {
              text: "IL 中间语言",
              icon: "fa6-solid:microchip",
              prefix: "IL中间语言/",
              collapsible: true,
              children: [
                "",
                {
                  text: "01 · 概述篇",
                  icon: "fa6-solid:book-open",
                  prefix: "01_概述篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "02 · 工具链篇",
                  icon: "fa6-solid:wrench",
                  prefix: "02_工具链篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "03 · 加载与存储",
                  icon: "fa6-solid:arrow-right-arrow-left",
                  prefix: "03_加载与存储/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "04 · 算术与位运算",
                  icon: "fa6-solid:calculator",
                  prefix: "04_算术与位运算/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "05 · 比较与控制流",
                  icon: "fa6-solid:code-branch",
                  prefix: "05_比较与控制流/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "06 · 类型转换",
                  icon: "fa6-solid:right-left",
                  prefix: "06_类型转换/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "07 · 方法调用",
                  icon: "fa6-solid:phone",
                  prefix: "07_方法调用/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "08 · 对象与类型",
                  icon: "fa6-solid:cubes",
                  prefix: "08_对象与类型/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "09 · 异常处理",
                  icon: "fa6-solid:triangle-exclamation",
                  prefix: "09_异常处理/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "10 · 泛型与IL",
                  icon: "fa6-solid:layer-group",
                  prefix: "10_泛型与IL/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "11 · 委托与函数指针",
                  icon: "fa6-solid:link",
                  prefix: "11_委托与函数指针/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "12 · 动态发射",
                  icon: "fa6-solid:rocket",
                  prefix: "12_动态发射/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "13 · 实战篇",
                  icon: "fa6-solid:flask",
                  prefix: "13_实战篇/",
                  collapsible: true,
                  children: "structure",
                },
              ],
            },
            {
              text: "CLR 深度",
              icon: "fa6-solid:microchip",
              prefix: "CLR深度/",
              collapsible: true,
              children: [
                "",
                {
                  text: "01 · CLR 基础与类型系统",
                  icon: "fa6-solid:cube",
                  prefix: "01_CLR基础与类型系统/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "02 · 对象与类型原理",
                  icon: "fa6-solid:layer-group",
                  prefix: "02_对象与类型原理/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "03 · 方法与委托原理",
                  icon: "fa6-solid:code",
                  prefix: "03_方法与委托原理/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "04 · 接口与泛型原理",
                  icon: "fa6-solid:puzzle-piece",
                  prefix: "04_接口与泛型原理/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "05 · 内存管理与垃圾回收",
                  icon: "fa6-solid:memory",
                  prefix: "05_内存管理与垃圾回收/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "06 · 异常处理与线程",
                  icon: "fa6-solid:triangle-exclamation",
                  prefix: "06_异常处理与线程/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "07 · 异步编程原理",
                  icon: "fa6-solid:rotate",
                  prefix: "07_异步编程原理/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "08 · 高级类型与模式",
                  icon: "fa6-solid:star",
                  prefix: "08_高级类型与模式/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "09 · 性能与诊断",
                  icon: "fa6-solid:gauge-high",
                  prefix: "09_性能与诊断/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "10 · 实战篇",
                  icon: "fa6-solid:flask",
                  prefix: "10_实战篇/",
                  collapsible: true,
                  children: "structure",
                },
              ],
            },
            {
              text: "Linq",
              icon: "fa6-solid:filter",
              prefix: "Linq/",
              collapsible: true,
              children: [
                "",
                {
                  text: "01 · 基础篇",
                  icon: "fa6-solid:seedling",
                  prefix: "01_基础篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "02 · 投影与筛选",
                  icon: "fa6-solid:filter",
                  prefix: "02_投影与筛选/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "03 · 排序与分组",
                  icon: "fa6-solid:arrow-down-wide-short",
                  prefix: "03_排序与分组/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "04 · 聚合与统计",
                  icon: "fa6-solid:calculator",
                  prefix: "04_聚合与统计/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "05 · 集合与连接",
                  icon: "fa6-solid:link",
                  prefix: "05_集合与连接/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "06 · 元素与分区",
                  icon: "fa6-solid:scissors",
                  prefix: "06_元素与分区/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "07 · 生成与转换",
                  icon: "fa6-solid:rotate",
                  prefix: "07_生成与转换/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "08 · 高级操作",
                  icon: "fa6-solid:rocket",
                  prefix: "08_高级操作/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "09 · 性能与陷阱",
                  icon: "fa6-solid:triangle-exclamation",
                  prefix: "09_性能与陷阱/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "10 · 实战篇",
                  icon: "fa6-solid:flask",
                  prefix: "10_实战篇/",
                  collapsible: true,
                  children: "structure",
                },
              ],
            },
          ],
        },
        {
          text: "数据库",
          icon: "fa6-solid:database",
          prefix: "数据库/",
          collapsible: true,
          children: [
            "",
            {
              text: "MySQL",
              icon: "fa6-brands:mysql",
              prefix: "MySQL/",
              collapsible: true,
              children: [
                "",
                {
                  text: "01 · 基础篇",
                  icon: "fa6-solid:cube",
                  prefix: "01_基础篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "02 · 进阶篇",
                  icon: "fa6-solid:layer-group",
                  prefix: "02_进阶篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "03 · 引擎篇",
                  icon: "fa6-solid:gear",
                  prefix: "03_引擎篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "04 · 运维篇",
                  icon: "fa6-solid:wrench",
                  prefix: "04_运维篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "05 · 实战篇",
                  icon: "fa6-solid:flask",
                  prefix: "05_实战篇/",
                  collapsible: true,
                  children: "structure",
                },
              ],
            },
            {
              text: "PostgreSQL",
              icon: "fa6-solid:elephant",
              prefix: "PostgreSQL/",
              collapsible: true,
              children: [
                "",
                {
                  text: "01 · 基础篇",
                  icon: "fa6-solid:cube",
                  prefix: "01_基础篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "02 · 进阶篇",
                  icon: "fa6-solid:layer-group",
                  prefix: "02_进阶篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "03 · 运维篇",
                  icon: "fa6-solid:wrench",
                  prefix: "03_运维篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "04 · 实战篇",
                  icon: "fa6-solid:flask",
                  prefix: "04_实战篇/",
                  collapsible: true,
                  children: "structure",
                },
              ],
            },
            {
              text: "SQL Server",
              icon: "fa6-brands:microsoft",
              prefix: "SQLServer/",
              collapsible: true,
              children: [
                "",
                {
                  text: "01 · 基础篇",
                  icon: "fa6-solid:cube",
                  prefix: "01_基础篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "02 · 进阶篇",
                  icon: "fa6-solid:layer-group",
                  prefix: "02_进阶篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "03 · 运维篇",
                  icon: "fa6-solid:wrench",
                  prefix: "03_运维篇/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "04 · 实战篇",
                  icon: "fa6-solid:flask",
                  prefix: "04_实战篇/",
                  collapsible: true,
                  children: "structure",
                },
              ],
            },
          ],
        },
        {
          text: ".NET IoT",
          icon: "fa6-solid:microchip",
          prefix: "DotNet_IoT/",
          collapsible: true,
          children: [
            "",
            {
              text: "01 · 物联网基础",
              icon: "fa6-solid:globe",
              prefix: "01_物联网基础/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "02 · 硬件接口与 GPIO",
              icon: "fa6-solid:microchip",
              prefix: "02_硬件接口与GPIO/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "03 · 通信协议",
              icon: "fa6-solid:network-wired",
              prefix: "03_通信协议/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "04 · 传感器与执行器",
              icon: "fa6-solid:temperature-half",
              prefix: "04_传感器与执行器/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "05 · 边缘计算与网关",
              icon: "fa6-solid:server",
              prefix: "05_边缘计算与网关/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "06 · IoT 平台与云服务",
              icon: "fa6-solid:cloud",
              prefix: "06_IoT平台与云服务/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "07 · 安全与认证",
              icon: "fa6-solid:shield-halved",
              prefix: "07_安全与认证/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "08 · 数据管道与分析",
              icon: "fa6-solid:chart-line",
              prefix: "08_数据管道与分析/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "09 · 生产级项目实战",
              icon: "fa6-solid:flask",
              prefix: "09_生产级项目实战/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "10 · 职业发展与面试",
              icon: "fa6-solid:briefcase",
              prefix: "10_职业发展与面试/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
        {
          text: "RabbitMQ",
          icon: "fa6-solid:envelope",
          prefix: "RabbitMQ/",
          collapsible: true,
          children: [
            "",
            {
              text: "01 · RabbitMQ 基础",
              icon: "fa6-solid:cube",
              prefix: "01_RabbitMQ基础/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "02 · 核心概念与架构",
              icon: "fa6-solid:sitemap",
              prefix: "02_核心概念与架构/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "03 · 交换机与路由",
              icon: "fa6-solid:route",
              prefix: "03_交换机与路由/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "04 · 消息可靠性",
              icon: "fa6-solid:shield-halved",
              prefix: "04_消息可靠性/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "05 · 高级特性",
              icon: "fa6-solid:star",
              prefix: "05_高级特性/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "06 · 集群与高可用",
              icon: "fa6-solid:server",
              prefix: "06_集群与高可用/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "07 · 性能调优",
              icon: "fa6-solid:gauge-high",
              prefix: "07_性能调优/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "08 · .NET 实战",
              icon: "fa6-brands:microsoft",
              prefix: "08_.NET实战/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "09 · 生产级架构设计",
              icon: "fa6-solid:building",
              prefix: "09_生产级架构设计/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "10 · 面试题精选",
              icon: "fa6-solid:file-circle-question",
              prefix: "10_面试题精选/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
        {
          text: "ABP框架",
          icon: "fa6-brands:microsoft",
          prefix: "ABP框架/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "ASP.NET Core",
          icon: "fa6-brands:microsoft",
          prefix: "ASP.NET_Core/",
          collapsible: true,
          children: [
            "",
            {
              text: "ASP.NET Core 底层原理",
              icon: "fa6-solid:circle-nodes",
              prefix: "AspNetCore底层原理/",
              collapsible: true,
              children: [
                "",
                "00_全链路总览",
                "01_网络层_TCP到Socket",
                "02_Pipelines零拷贝内存模型",
                "03_HTTP协议解析状态机",
                "04_IFeatureCollection接口壁垒",
                "05_HostingApplication交接点",
                "06_中间件管道洋葱圈",
                "07_路由系统DFA匹配",
                "08_认证授权",
                "09_模型绑定",
                "10_Action执行与过滤器",
                "11_输出格式化与响应写回",
                "12_响应返回网卡",
              ],
            },
            {
              text: "提问式学习 ASP.NET Core",
              icon: "fa6-solid:comments",
              prefix: "提问式学习asp.netcore/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "单点登录",
              icon: "fa6-solid:key",
              prefix: "单点登录/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "领域驱动设计",
              icon: "fa6-solid:diagram-project",
              prefix: "领域驱动设计/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "Mok 基础架构设施",
              icon: "fa6-solid:layer-group",
              prefix: "Mok基础架构设施/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "SQL Server 进阶",
              icon: "fa6-solid:database",
              prefix: "SQL Server进阶/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "SQL 知识库",
              icon: "fa6-solid:table",
              prefix: "SQL知识库/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "中间件开发",
              icon: "fa6-solid:gear",
              prefix: "中间件开发/",
              collapsible: true,
              children: [
                "",
                {
                  text: "01 · 中间件基础",
                  icon: "fa6-solid:seedling",
                  prefix: "01_中间件基础/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "02 · 常用中间件实战",
                  icon: "fa6-solid:code",
                  prefix: "02_常用中间件实战/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "03 · 高级中间件实战",
                  icon: "fa6-solid:rocket",
                  prefix: "03_高级中间件实战/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "04 · 中间件设计模式",
                  icon: "fa6-solid:puzzle-piece",
                  prefix: "04_中间件设计模式/",
                  collapsible: true,
                  children: "structure",
                },
              ],
            },
          ],
        },
        // 算法单独显式声明，保证七个模块顺序固定
        {
          text: "C# 算法知识库",
          icon: "fa6-solid:code",
          prefix: "算法/",
          collapsible: true,
          children: [
            "",
            {
              text: "01. 数据结构基础",
              icon: "fa6-solid:cube",
              prefix: "01_数据结构基础/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "02. 排序算法",
              icon: "fa6-solid:arrow-down-short-wide",
              prefix: "02_排序算法/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "03. 查找算法",
              icon: "fa6-solid:magnifying-glass",
              prefix: "03_查找算法/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "04. 核心算法思想",
              icon: "fa6-solid:lightbulb",
              prefix: "04_核心算法思想/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "05. 树与图算法",
              icon: "fa6-solid:diagram-project",
              prefix: "05_树与图算法/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "06. 字符串算法",
              icon: "fa6-solid:font",
              prefix: "06_字符串算法/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "07. 数学算法",
              icon: "fa6-solid:calculator",
              prefix: "07_数学算法/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
        {
          text: "Rust",
          icon: "fa6-brands:rust",
          prefix: "Rust/",
          collapsible: true,
          children: [
            "",
            {
              text: "Rust基础",
              icon: "fa6-solid:graduation-cap",
              prefix: "Rust基础/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "MoCLI · CLI 工具",
              icon: "fa6-solid:terminal",
              prefix: "MoCLI/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "Restrel · Web 服务服务器",
              icon: "fa6-solid:server",
              prefix: "Restrel/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "MoNode · 嵌入式",
              icon: "fa6-solid:microchip",
              prefix: "MoNode/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
        // 权限设计单独显式声明，保证六个模块顺序固定
        {
          text: "权限系统设计",
          icon: "fa6-solid:shield-halved",
          prefix: "权限设计/",
          collapsible: true,
          children: [
            "",
            {
              text: "01. 权限模型演进",
              icon: "fa6-solid:timeline",
              prefix: "01_权限模型演进/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "02. RBAC 深度解析",
              icon: "fa6-solid:id-badge",
              prefix: "02_RBAC深度解析/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "03. ABAC 细粒度控制",
              icon: "fa6-solid:filter",
              prefix: "03_ABAC细粒度控制/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "04. PBAC 策略引擎",
              icon: "fa6-solid:gavel",
              prefix: "04_PBAC策略引擎/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "05. 混合架构落地",
              icon: "fa6-solid:layer-group",
              prefix: "05_混合架构落地/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "06. 性能与工程实践",
              icon: "fa6-solid:gauge-high",
              prefix: "06_性能与工程实践/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
        {
          text: "Redis",
          icon: "fa6-solid:bolt",
          prefix: "Redis/",
          collapsible: true,
          children: [
            "",
            {
              text: "01 · Redis 基础",
              icon: "fa6-solid:cube",
              prefix: "01_Redis基础/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "02 · 数据类型与结构",
              icon: "fa6-solid:layer-group",
              prefix: "02_数据类型与结构/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "03 · 持久化机制",
              icon: "fa6-solid:hard-drive",
              prefix: "03_持久化机制/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "04 · 复制与高可用",
              icon: "fa6-solid:server",
              prefix: "04_复制与高可用/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "05 · 集群与分片",
              icon: "fa6-solid:network-wired",
              prefix: "05_集群与分片/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "06 · 内存管理与性能",
              icon: "fa6-solid:gauge-high",
              prefix: "06_内存管理与性能/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "07 · 高级特性",
              icon: "fa6-solid:star",
              prefix: "07_高级特性/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "08 · .NET 实战",
              icon: "fa6-brands:microsoft",
              prefix: "08_.NET实战/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "09 · 生产级架构设计",
              icon: "fa6-solid:building",
              prefix: "09_生产级架构设计/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "10 · 面试题精选",
              icon: "fa6-solid:file-circle-question",
              prefix: "10_面试题精选/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
      ],
    },
    {
      text: "前端开发",
      icon: "fa6-solid:laptop-code",
      prefix: "前端开发/",
      collapsible: true,
      children: [
        "",
        {
          text: "ES6 教程",
          icon: "fa6-solid:code",
          prefix: "ES6教程/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "JavaScript 教程",
          icon: "fa6-brands:js",
          prefix: "JavaScript教程/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "TypeScript 从零实现 axios",
          icon: "fa6-solid:code-branch",
          prefix: "TypeScript 从零实现 axios/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "React 19 教程",
          icon: "fa6-brands:react",
          prefix: "react19教程/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "React 中后台架构知识库",
          icon: "fa6-solid:sitemap",
          prefix: "React中后台架构知识库/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "TailwindCSS",
          icon: "fa6-solid:palette",
          prefix: "TailwindCSS/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "TailwindCSS 知识库",
          icon: "fa6-solid:swatchbook",
          prefix: "TailwindCSS知识库/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "SaaS 实战开发",
          icon: "fa6-solid:building",
          prefix: "Saas实战/",
          collapsible: true,
          children: [
            "",
            {
              text: "01 · React 入门与环境搭建",
              icon: "fa6-solid:rocket",
              prefix: "01-React入门与环境搭建/",
              collapsible: true,
              children: [
                "",
                "01-Node与包管理器",
                "02-创建Vite项目",
                "03-安装核心依赖",
                "04-配置Vite与TypeScript",
                "05-shadcn-ui初始化",
                "06-全局样式与字体",
                "07-启动与验证",
              ],
            },
            {
              text: "02 · 项目架构与工程化",
              icon: "fa6-solid:sitemap",
              prefix: "02-项目架构与工程化/",
              collapsible: true,
              children: [
                "",
                "01-目录结构设计",
                "02-工具函数",
                "03-HTTP客户端封装",
                "04-路由系统",
                "05-布局组件",
                "06-状态管理",
                "07-类型定义体系",
              ],
            },
            {
              text: "03 · SSO 单点登录",
              icon: "fa6-solid:key",
              prefix: "03-SSO单点登录/",
              collapsible: true,
              children: [
                "",
                "01-OAuth2与JWT基础",
                "02-登录页面",
                "03-认证API层",
                "04-useAuth-Hook",
                "05-Token管理",
                "06-路由守卫",
                "07-租户选择",
              ],
            },
            {
              text: "04 · 权限中心",
              icon: "fa6-solid:shield-halved",
              prefix: "04-权限中心/",
              collapsible: true,
              children: [
                "",
                "01-RBAC模型与权限树",
                "02-权限API层",
                "03-应用管理",
                "04-权限定义",
                "05-角色管理",
                "06-动态菜单",
                "07-用户角色分配",
                "08-组织架构",
                "09-成员管理",
              ],
            },
            {
              text: "05 · API 网关管理",
              icon: "fa6-solid:network-wired",
              prefix: "05-API网关管理/",
              collapsible: true,
              children: [
                "",
                "01-YARP网关基础",
                "02-网关HTTP客户端",
                "03-集群管理",
                "04-路由管理",
                "05-限流策略",
                "06-网关仪表盘",
              ],
            },
            {
              text: "06 · 文件服务",
              icon: "fa6-solid:file",
              prefix: "06-文件服务/",
              collapsible: true,
              children: [
                "",
                "01-文件服务架构",
                "02-文件API层",
                "03-文件管理器",
                "04-文件上传",
                "05-文件预览",
                "06-配额管理",
                "07-访问控制",
                "08-文件分享",
                "09-回收站",
              ],
            },
            {
              text: "07 · 打印服务",
              icon: "fa6-solid:print",
              prefix: "07-打印服务/",
              collapsible: true,
              children: [
                "",
                "01-模块概览与路由配置",
                "02-API层-模板数据源任务",
                "03-类型系统定义",
                "04-数据源管理页面",
                "05-模板与任务管理页面",
              ],
            },
            {
              text: "08 · 通知中心",
              icon: "fa6-solid:bell",
              prefix: "08-通知中心/",
              collapsible: true,
              children: [
                "",
                "01-实时通信基础",
                "02-SignalR封装",
                "03-通知API层",
                "04-通知铃铛",
                "05-通知列表页",
                "06-模板管理",
                "07-偏好设置",
              ],
            },
            {
              text: "09 · 配置中心",
              icon: "fa6-solid:gears",
              prefix: "09-配置中心/",
              collapsible: true,
              children: [
                "",
                "01-运行时配置概述",
                "02-路由与类型定义",
                "03-配置页面组件",
                "04-加密值交互逻辑",
                "05-Vite代理与最佳实践",
              ],
            },
            {
              text: "10 · 租户运营中心",
              icon: "fa6-solid:building-columns",
              prefix: "10-租户运营中心/",
              collapsible: true,
              children: [
                "",
                "01-SaaS多租户架构",
                "02-运营API层",
                "03-平台概览",
                "04-版本套餐管理",
                "05-租户管理",
                "06-租户详情",
                "07-功能开关",
                "08-Webhook管理",
                "09-MFA管理",
                "10-数据交换",
                "11-工作流",
              ],
            },
            {
              text: "11 · CSS 布局实战",
              icon: "fa6-solid:table-columns",
              prefix: "11-CSS布局实战/",
              collapsible: true,
              children: [
                "",
                "00-布局速查手册",
                "01-登录页居中布局",
                "02-管理后台框架布局",
                "03-数据列表页布局",
                "04-详情页Tab布局",
                "05-表单页布局",
                "06-文件管理器布局",
                "07-仪表盘网格布局",
              ],
            },
          ],
        },
      ],
    },
    {
      text: "架构与设计",
      icon: "fa6-solid:sitemap",
      prefix: "架构与设计/",
      collapsible: true,
      children: "structure",
    },
    {
      text: "计算机学科",
      icon: "fa6-solid:graduation-cap",
      prefix: "计算机学科/",
      collapsible: true,
      children: [
        {
          text: "数字电路",
          icon: "fa6-solid:toggle-on",
          prefix: "数字电路/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "计算机组成原理",
          icon: "fa6-solid:memory",
          prefix: "计算机组成原理/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "汇编语言",
          icon: "fa6-solid:microchip",
          prefix: "汇编语言/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "操作系统",
          icon: "fa6-solid:desktop",
          prefix: "操作系统/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "计算机网络",
          icon: "fa6-solid:network-wired",
          prefix: "计算机网络/",
          collapsible: true,
          children: "structure",
        },
      ],
    },
    {
      text: "AI 实践",
      icon: "fa6-solid:robot",
      prefix: "AI实践/",
      collapsible: true,
      children: [
        {
          text: "第一阶段：基础",
          icon: "fa6-solid:seedling",
          prefix: "01_基础阶段/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "第二阶段：核心",
          icon: "fa6-solid:fire",
          prefix: "02_核心阶段/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "第三阶段：进阶",
          icon: "fa6-solid:bolt",
          prefix: "03_进阶阶段/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "第四阶段：架构",
          icon: "fa6-solid:building-columns",
          prefix: "04_架构阶段/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "第五阶段：业务落地",
          icon: "fa6-solid:briefcase",
          prefix: "05_业务落地/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "AI 演进史",
          icon: "fa6-solid:clock-rotate-left",
          prefix: "06_AI演进史/",
          collapsible: true,
          children: [
            "",
            {
              text: "符号主义与早期探索",
              icon: "fa6-solid:scroll",
              prefix: "01_符号主义时代/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "机器学习兴起",
              icon: "fa6-solid:chart-line",
              prefix: "02_机器学习兴起/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "深度学习革命",
              icon: "fa6-solid:brain",
              prefix: "03_深度学习革命/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "Transformer 时代",
              icon: "fa6-solid:wand-magic-sparkles",
              prefix: "04_Transformer时代/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "大语言模型时代",
              icon: "fa6-solid:microchip",
              prefix: "05_大语言模型时代/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "Agent 与未来",
              icon: "fa6-solid:rocket",
              prefix: "06_Agent与未来/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
        {
          text: "AI 文档知识库与客服",
          icon: "fa6-solid:book-open-reader",
          prefix: "AI文档库与客服/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "AI 工程师专栏",
          icon: "fa6-solid:user-astronaut",
          prefix: "07_AI工程师专栏/",
          collapsible: true,
          children: [
            {
              text: "岗位画像与能力模型",
              icon: "fa6-solid:map",
              prefix: "01_岗位画像与能力模型/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "Prompt 工程深度实践",
              icon: "fa6-solid:pen-fancy",
              prefix: "02_Prompt工程深度实践/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "RAG 系统工程化",
              icon: "fa6-solid:magnifying-glass",
              prefix: "03_RAG系统工程化/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "Agent 系统开发",
              icon: "fa6-solid:sitemap",
              prefix: "04_Agent系统开发/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "模型微调与推理优化",
              icon: "fa6-solid:sliders",
              prefix: "05_模型微调与推理优化/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "LLMOps 与生产化",
              icon: "fa6-solid:gears",
              prefix: "06_LLMOps与生产化/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "实战项目与求职",
              icon: "fa6-solid:briefcase",
              prefix: "07_实战项目与求职/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
        {
          text: "行业 AI 解决方案",
          icon: "fa6-solid:building",
          prefix: "08_行业AI解决方案/",
          collapsible: true,
          children: [
            {
              text: "金融 AI",
              icon: "fa6-solid:landmark",
              prefix: "01_金融AI/",
              collapsible: true,
              children: [
                {
                  text: "金融行业知识",
                  icon: "fa6-solid:book",
                  prefix: "01_金融行业知识/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "金融 AI 技术",
                  icon: "fa6-solid:microchip",
                  prefix: "02_金融AI技术/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "反欺诈与合规系统",
                  icon: "fa6-solid:shield-halved",
                  prefix: "03_反欺诈与合规系统/",
                  collapsible: true,
                  children: "structure",
                },
              ],
            },
            {
              text: "医疗 AI",
              icon: "fa6-solid:stethoscope",
              prefix: "02_医疗AI/",
              collapsible: true,
              children: [
                {
                  text: "医疗行业知识",
                  icon: "fa6-solid:book",
                  prefix: "01_医疗行业知识/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "医疗 AI 技术",
                  icon: "fa6-solid:microchip",
                  prefix: "02_医疗AI技术/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "智慧诊疗系统",
                  icon: "fa6-solid:hospital",
                  prefix: "03_智慧诊疗系统/",
                  collapsible: true,
                  children: "structure",
                },
              ],
            },
            {
              text: "电商 AI",
              icon: "fa6-solid:cart-shopping",
              prefix: "03_电商AI/",
              collapsible: true,
              children: [
                {
                  text: "电商行业知识",
                  icon: "fa6-solid:book",
                  prefix: "01_电商行业知识/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "电商 AI 技术",
                  icon: "fa6-solid:microchip",
                  prefix: "02_电商AI技术/",
                  collapsible: true,
                  children: "structure",
                },
                {
                  text: "智能供应链系统",
                  icon: "fa6-solid:truck",
                  prefix: "03_智能供应链系统/",
                  collapsible: true,
                  children: "structure",
                },
              ],
            },
          ],
        },
        {
          text: "Microsoft Agent Framework 实战",
          icon: "fa6-solid:robot",
          prefix: "AgentFramework/",
          collapsible: true,
          children: "structure",
        },
      ],
    },
    {
      text: "AI 书籍",
      icon: "fa6-solid:book",
      collapsible: true,
      children: [
        {
          text: "AI 知识库",
          icon: "fa6-solid:book-open-reader",
          prefix: "AI知识库/",
          collapsible: true,
          children: [
            "",
            "ai-core-concepts",
            {
              text: "大模型基础",
              icon: "fa6-solid:brain",
              prefix: "llm-basis/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "AI Agent",
              icon: "fa6-solid:robot",
              prefix: "agent/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "RAG 检索增强生成",
              icon: "fa6-solid:magnifying-glass",
              prefix: "rag/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "AI 系统设计",
              icon: "fa6-solid:sitemap",
              prefix: "system-design/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "AI 面试题",
              icon: "fa6-solid:file-circle-question",
              prefix: "interview-questions/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
        {
          text: "从零构建智能体",
          icon: "fa6-solid:robot",
          prefix: "从零构建智能体/",
          collapsible: true,
          sidebarSorter: ["readme", "order", "filename"],
          children: [
            "",
            { text: "前言", icon: "fa6-solid:book-open", link: "00-前言" },
            "01-初识智能体",
            "02-智能体发展史",
            "03-大语言模型基础",
            "04-智能体经典范式构建",
            "05-基于低代码平台的智能体搭建",
            "06-框架开发实践",
            "07-构建你的Agent框架",
            "08-记忆与检索",
            "09-上下文工程",
            "10-智能体通信协议",
            "11-Agentic-RL",
            "12-智能体性能评估",
            "13-智能旅行助手",
            "14-自动化深度研究智能体",
            "15-构建赛博小镇",
            "16-毕业设计",
            {
              text: "番外篇",
              icon: "fa6-solid:star",
              prefix: "番外篇/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: "structure",
            },
          ],
        },
        {
          text: "深入理解 AI Agent",
          icon: "fa6-solid:wand-magic-sparkles",
          prefix: "深入理解AIAgent/",
          collapsible: true,
          children: [
            "",
            "00-引言",
            "01-AI-Agent-入门",
            "02-上下文工程",
            "03-用户记忆和知识库",
            "04-工具",
            "05-Coding-Agent-与通用-Agent",
            "06-交互：观察与动作空间的扩展",
            "07-Agent-的评估",
            "08-模型后训练",
            "09-Agent-的持续进化",
            "10-多-Agent-协作",
            "11-后记",
            "12-思考题参考答案",
          ],
        },
        {
          text: "动手学深度学习",
          icon: "fa6-solid:brain",
          prefix: "动手学深度学习/",
          collapsible: true,
          children: [
            "",
            {
              text: "00-序言",
              prefix: "00-序言/",
              collapsible: true,
              children: [
              ],
            },
            {
              text: "01-安装",
              prefix: "01-安装/",
              collapsible: true,
              children: [
              ],
            },
            {
              text: "02-符号",
              prefix: "02-符号/",
              collapsible: true,
              children: [
              ],
            },
            {
              text: "03-引言",
              prefix: "03-引言/",
              collapsible: true,
              children: [
              ],
            },
            {
              text: "04-预备知识",
              prefix: "04-预备知识/",
              collapsible: true,
              children: [
                "01-ndarray",
                "02-pandas",
                "03-linear-algebra",
                "04-calculus",
                "05-autograd",
                "06-probability",
                "07-lookup-api",
              ],
            },
            {
              text: "05-线性神经网络",
              prefix: "05-线性神经网络/",
              collapsible: true,
              children: [
                "01-linear-regression",
                "02-linear-regression-scratch",
                "03-linear-regression-concise",
                "04-softmax-regression",
                "05-image-classification-dataset",
                "06-softmax-regression-scratch",
                "07-softmax-regression-concise",
              ],
            },
            {
              text: "06-多层感知机",
              prefix: "06-多层感知机/",
              collapsible: true,
              children: [
                "01-mlp",
                "02-mlp-scratch",
                "03-mlp-concise",
                "04-underfit-overfit",
                "05-weight-decay",
                "06-dropout",
                "07-backprop",
                "08-numerical-stability-and-init",
                "09-environment",
                "10-kaggle-house-price",
              ],
            },
            {
              text: "07-深度学习计算",
              prefix: "07-深度学习计算/",
              collapsible: true,
              children: [
                "01-model-construction",
                "02-parameters",
                "03-deferred-init",
                "04-custom-layer",
                "05-read-write",
                "06-use-gpu",
              ],
            },
            {
              text: "08-卷积神经网络",
              prefix: "08-卷积神经网络/",
              collapsible: true,
              children: [
                "01-why-conv",
                "02-conv-layer",
                "03-padding-and-strides",
                "04-channels",
                "05-pooling",
                "06-lenet",
              ],
            },
            {
              text: "09-现代卷积神经网络",
              prefix: "09-现代卷积神经网络/",
              collapsible: true,
              children: [
                "01-alexnet",
                "02-vgg",
                "03-nin",
                "04-googlenet",
                "05-batch-norm",
                "06-resnet",
                "07-densenet",
              ],
            },
            {
              text: "10-循环神经网络",
              prefix: "10-循环神经网络/",
              collapsible: true,
              children: [
                "01-sequence",
                "02-text-preprocessing",
                "03-language-models-and-dataset",
                "04-rnn",
                "05-rnn-scratch",
                "06-rnn-concise",
                "07-bptt",
              ],
            },
            {
              text: "11-现代循环神经网络",
              prefix: "11-现代循环神经网络/",
              collapsible: true,
              children: [
                "01-gru",
                "02-lstm",
                "03-deep-rnn",
                "04-bi-rnn",
                "05-machine-translation-and-dataset",
                "06-encoder-decoder",
                "07-seq2seq",
                "08-beam-search",
              ],
            },
            {
              text: "12-注意力机制",
              prefix: "12-注意力机制/",
              collapsible: true,
              children: [
                "01-attention-cues",
                "02-nadaraya-waston",
                "03-attention-scoring-functions",
                "04-bahdanau-attention",
                "05-multihead-attention",
                "06-self-attention-and-positional-encoding",
                "07-transformer",
              ],
            },
            {
              text: "13-优化算法",
              prefix: "13-优化算法/",
              collapsible: true,
              children: [
                "01-optimization-intro",
                "02-convexity",
                "03-gd",
                "04-sgd",
                "05-minibatch-sgd",
                "06-momentum",
                "07-adagrad",
                "08-rmsprop",
                "09-adadelta",
                "10-adam",
                "11-lr-scheduler",
              ],
            },
            {
              text: "14-计算性能",
              prefix: "14-计算性能/",
              collapsible: true,
              children: [
                "01-hybridize",
                "02-async-computation",
                "03-auto-parallelism",
                "04-hardware",
                "05-multiple-gpus",
                "06-multiple-gpus-concise",
                "07-parameterserver",
              ],
            },
            {
              text: "15-计算机视觉",
              prefix: "15-计算机视觉/",
              collapsible: true,
              children: [
                "01-image-augmentation",
                "02-fine-tuning",
                "03-bounding-box",
                "04-anchor",
                "05-multiscale-object-detection",
                "06-object-detection-dataset",
                "07-ssd",
                "08-rcnn",
                "09-semantic-segmentation-and-dataset",
                "10-transposed-conv",
                "11-fcn",
                "12-neural-style",
                "13-kaggle-cifar10",
                "14-kaggle-dog",
              ],
            },
            {
              text: "16-自然语言处理-预训练",
              prefix: "16-自然语言处理-预训练/",
              collapsible: true,
              children: [
                "01-word2vec",
                "02-approx-training",
                "03-word-embedding-dataset",
                "04-word2vec-pretraining",
                "05-glove",
                "06-subword-embedding",
                "07-similarity-analogy",
                "08-bert",
                "09-bert-dataset",
                "10-bert-pretraining",
              ],
            },
            {
              text: "17-自然语言处理-应用",
              prefix: "17-自然语言处理-应用/",
              collapsible: true,
              children: [
                "01-sentiment-analysis-and-dataset",
                "02-sentiment-analysis-rnn",
                "03-sentiment-analysis-cnn",
                "04-natural-language-inference-and-dataset",
                "05-natural-language-inference-attention",
                "06-finetuning-bert",
                "07-natural-language-inference-bert",
              ],
            },
            {
              text: "18-附录-深度学习工具",
              prefix: "18-附录-深度学习工具/",
              collapsible: true,
              children: [
                "01-jupyter",
                "02-sagemaker",
                "03-aws",
                "04-selecting-servers-gpus",
                "05-contributing",
                "06-d2l",
              ],
            },
          ],
        },
        {
          text: "AI 工程实战(从零到生产)",
          icon: "fa6-solid:gears",
          prefix: "ai-eng/",
          collapsible: true,
          sidebarSorter: ["readme", "order", "filename"],
          children: [
            "",
            {
              text: "第 0 阶段 · 环境与工具",
              prefix: "00-setup-and-tooling/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-dev-environment",
                "02-git-and-collaboration",
                "03-gpu-setup-and-cloud",
                "04-apis-and-keys",
                "05-jupyter-notebooks",
                "06-python-environments",
                "07-docker-for-ai",
                "08-editor-setup",
                "09-data-management",
                "10-terminal-and-shell",
                "11-linux-for-ai",
                "12-debugging-and-profiling",
              ],
            },
            {
              text: "第 1 阶段 · 数学基础",
              prefix: "01-math-foundations/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-linear-algebra-intuition",
                "02-vectors-matrices-operations",
                "03-matrix-transformations",
                "04-calculus-for-ml",
                "05-chain-rule-and-autodiff",
                "06-probability-and-distributions",
                "07-bayes-theorem",
                "08-optimization",
                "09-information-theory",
                "10-dimensionality-reduction",
                "11-singular-value-decomposition",
                "12-tensor-operations",
                "13-numerical-stability",
                "14-norms-and-distances",
                "15-statistics-for-ml",
                "16-sampling-methods",
                "17-linear-systems",
                "18-convex-optimization",
                "19-complex-numbers",
                "20-fourier-transform",
                "21-graph-theory",
                "22-stochastic-processes",
              ],
            },
            {
              text: "第 2 阶段 · 机器学习基础",
              prefix: "02-ml-fundamentals/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-what-is-machine-learning",
                "02-linear-regression",
                "03-logistic-regression",
                "04-decision-trees",
                "05-support-vector-machines",
                "06-knn-and-distances",
                "07-unsupervised-learning",
                "08-feature-engineering",
                "09-model-evaluation",
                "10-bias-variance",
                "11-ensemble-methods",
                "12-hyperparameter-tuning",
                "13-ml-pipelines",
                "14-naive-bayes",
                "15-time-series",
                "16-anomaly-detection",
                "17-imbalanced-data",
                "18-feature-selection",
              ],
            },
            {
              text: "第 3 阶段 · 深度学习核心",
              prefix: "03-deep-learning-core/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-the-perceptron",
                "02-multi-layer-networks",
                "03-backpropagation",
                "04-activation-functions",
                "05-loss-functions",
                "06-optimizers",
                "07-regularization",
                "08-weight-initialization",
                "09-learning-rate-schedules",
                "10-mini-framework",
                "11-intro-to-pytorch",
                "12-intro-to-jax",
                "13-debugging-neural-networks",
              ],
            },
            {
              text: "第 4 阶段 · 计算机视觉",
              prefix: "04-computer-vision/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-image-fundamentals",
                "02-convolutions-from-scratch",
                "03-cnns-lenet-to-resnet",
                "04-image-classification",
                "05-transfer-learning",
                "06-object-detection-yolo",
                "07-semantic-segmentation-unet",
                "08-instance-segmentation-mask-rcnn",
                "09-image-generation-gans",
                "10-image-generation-diffusion",
                "11-stable-diffusion",
                "12-video-understanding",
                "13-3d-vision-nerf",
                "14-vision-transformers",
                "15-real-time-edge",
                "16-vision-pipeline-capstone",
                "17-self-supervised-vision",
                "18-open-vocab-clip",
                "19-ocr-document-understanding",
                "20-image-retrieval-metric",
                "21-keypoint-pose",
                "22-3d-gaussian-splatting",
                "23-diffusion-transformers-rectified-flow",
                "24-sam3-open-vocab-segmentation",
                "25-vision-language-models",
                "26-monocular-depth",
                "27-multi-object-tracking",
                "28-world-models-video-diffusion",
              ],
            },
            {
              text: "第 5 阶段 · NLP 基础到进阶",
              prefix: "05-nlp-foundations-to-advanced/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-text-processing",
                "02-bag-of-words-tfidf",
                "03-word-embeddings-word2vec",
                "04-glove-fasttext-subword",
                "05-sentiment-analysis",
                "06-named-entity-recognition",
                "07-pos-tagging-parsing",
                "08-cnns-rnns-for-text",
                "09-sequence-to-sequence",
                "10-attention-mechanism",
                "11-machine-translation",
                "12-text-summarization",
                "13-question-answering",
                "14-information-retrieval-search",
                "15-topic-modeling",
                "16-text-generation-pre-transformer",
                "17-chatbots-rule-to-neural",
                "18-multilingual-nlp",
                "19-subword-tokenization",
                "20-structured-outputs-constrained-decoding",
                "21-nli-textual-entailment",
                "22-embedding-models-deep-dive",
                "23-chunking-strategies-rag",
                "24-coreference-resolution",
                "25-entity-linking",
                "26-relation-extraction-kg",
                "27-llm-evaluation-frameworks",
                "28-long-context-evaluation",
                "29-dialogue-state-tracking",
              ],
            },
            {
              text: "第 6 阶段 · 语音与音频",
              prefix: "06-speech-and-audio/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-audio-fundamentals",
                "02-spectrograms-mel-features",
                "03-audio-classification",
                "04-speech-recognition-asr",
                "05-whisper-architecture-finetuning",
                "06-speaker-recognition-verification",
                "07-text-to-speech",
                "08-voice-cloning-conversion",
                "09-music-generation",
                "10-audio-language-models",
                "11-real-time-audio-processing",
                "12-voice-assistant-pipeline",
                "13-neural-audio-codecs",
                "14-voice-activity-detection-turn-taking",
                "15-streaming-speech-to-speech-moshi-hibiki",
                "16-anti-spoofing-audio-watermarking",
                "17-audio-evaluation-metrics",
              ],
            },
            {
              text: "第 7 阶段 · Transformer 深入",
              prefix: "07-transformers-deep-dive/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-why-transformers",
                "02-self-attention-from-scratch",
                "03-multi-head-attention",
                "04-positional-encoding",
                "05-full-transformer",
                "06-bert-masked-language-modeling",
                "07-gpt-causal-language-modeling",
                "08-t5-bart-encoder-decoder",
                "09-vision-transformers",
                "10-audio-transformers-whisper",
                "11-mixture-of-experts",
                "12-kv-cache-flash-attention",
                "13-scaling-laws",
                "14-build-a-transformer-capstone",
                "15-attention-variants",
                "16-speculative-decoding",
              ],
            },
            {
              text: "第 8 阶段 · 生成式 AI",
              prefix: "08-generative-ai/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-generative-models-taxonomy-history",
                "02-autoencoders-vae",
                "03-gans-generator-discriminator",
                "04-conditional-gans-pix2pix",
                "05-stylegan",
                "06-diffusion-ddpm-from-scratch",
                "07-latent-diffusion-stable-diffusion",
                "08-controlnet-lora-conditioning",
                "09-inpainting-outpainting-editing",
                "10-video-generation",
                "11-audio-generation",
                "12-3d-generation",
                "13-flow-matching-rectified-flows",
                "14-evaluation-fid-clip-score",
                "19-visual-autoregressive-var",
              ],
            },
            {
              text: "第 9 阶段 · 强化学习",
              prefix: "09-reinforcement-learning/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-mdps-states-actions-rewards",
                "02-dynamic-programming",
                "03-monte-carlo-methods",
                "04-q-learning-sarsa",
                "05-dqn",
                "06-policy-gradients-reinforce",
                "07-actor-critic-a2c-a3c",
                "08-ppo",
                "09-reward-modeling-rlhf",
                "10-multi-agent-rl",
                "11-sim-to-real-transfer",
                "12-rl-for-games",
              ],
            },
            {
              text: "第 10 阶段 · 从零构建 LLM",
              prefix: "10-llms-from-scratch/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-tokenizers",
                "02-building-a-tokenizer",
                "03-data-pipelines",
                "04-pre-training-mini-gpt",
                "05-scaling-distributed",
                "06-instruction-tuning-sft",
                "07-rlhf",
                "08-dpo",
                "09-constitutional-ai-self-improvement",
                "10-evaluation",
                "11-quantization",
                "12-inference-optimization",
                "13-building-complete-llm-pipeline",
                "14-open-models-architecture-walkthroughs",
                "15-speculative-decoding-eagle3",
                "16-differential-attention-v2",
                "17-native-sparse-attention",
                "18-multi-token-prediction",
                "19-dualpipe-parallelism",
                "20-deepseek-v3-walkthrough",
                "21-jamba-hybrid-ssm-transformer",
                "22-async-hogwild-inference",
                "25-speculative-decoding",
                "34-gradient-checkpointing",
              ],
            },
            {
              text: "第 11 阶段 · LLM 工程",
              prefix: "11-llm-engineering/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-prompt-engineering",
                "02-few-shot-cot",
                "03-structured-outputs",
                "04-embeddings",
                "05-context-engineering",
                "06-rag",
                "07-advanced-rag",
                "08-fine-tuning-lora",
                "09-function-calling",
                "10-evaluation",
                "11-caching-cost",
                "12-guardrails",
                "13-production-app",
                "14-model-context-protocol",
                "15-prompt-caching",
                "16-langgraph-state-machines",
                "17-agent-framework-tradeoffs",
              ],
            },
            {
              text: "第 12 阶段 · 多模态 AI",
              prefix: "12-multimodal-ai/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-vision-transformer-patch-tokens",
                "02-clip-contrastive-pretraining",
                "03-blip2-qformer-bridge",
                "04-flamingo-gated-cross-attention",
                "05-llava-visual-instruction-tuning",
                "06-any-resolution-patch-n-pack",
                "07-open-weight-vlm-recipes",
                "08-llava-onevision-single-multi-video",
                "09-qwen-vl-family-dynamic-fps",
                "10-internvl3-native-multimodal",
                "11-chameleon-early-fusion-tokens",
                "12-emu3-next-token-for-generation",
                "13-transfusion-autoregressive-diffusion",
                "14-show-o-discrete-diffusion-unified",
                "15-janus-pro-decoupled-encoders",
                "16-mio-any-to-any-streaming",
                "17-video-language-temporal-grounding",
                "18-long-video-million-token",
                "19-audio-language-whisper-to-af3",
                "20-omni-models-thinker-talker",
                "21-embodied-vlas-openvla-pi0-groot",
                "22-document-diagram-understanding",
                "23-colpali-vision-native-rag",
                "24-multimodal-rag-cross-modal",
                "25-multimodal-agents-computer-use",
              ],
            },
            {
              text: "第 13 阶段 · 工具与协议",
              prefix: "13-tools-and-protocols/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-the-tool-interface",
                "02-function-calling-deep-dive",
                "03-parallel-and-streaming-tool-calls",
                "04-structured-output",
                "05-tool-schema-design",
                "06-mcp-fundamentals",
                "07-building-an-mcp-server",
                "08-building-an-mcp-client",
                "09-mcp-transports",
                "10-mcp-resources-and-prompts",
                "11-mcp-sampling",
                "12-mcp-roots-and-elicitation",
                "13-mcp-async-tasks",
                "14-mcp-apps",
                "15-mcp-security-tool-poisoning",
                "16-mcp-security-oauth-2-1",
                "17-mcp-gateways-and-registries",
                "18-mcp-auth-production",
                "19-a2a-protocol",
                "20-opentelemetry-genai",
                "21-llm-routing-layer",
                "22-skills-and-agent-sdks",
                "23-capstone-tool-ecosystem",
              ],
            },
            {
              text: "第 14 阶段 · 智能体工程",
              prefix: "14-agent-engineering/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-the-agent-loop",
                "02-rewoo-plan-and-execute",
                "03-reflexion-verbal-rl",
                "04-tree-of-thoughts-lats",
                "05-self-refine-and-critic",
                "06-tool-use-and-function-calling",
                "07-memory-virtual-context-memgpt",
                "08-memory-blocks-sleep-time-compute",
                "09-hybrid-memory-mem0",
                "10-skill-libraries-voyager",
                "11-planning-htn-and-evolutionary",
                "12-anthropic-workflow-patterns",
                "13-langgraph-stateful-graphs",
                "14-autogen-actor-model",
                "15-crewai-role-based-crews",
                "16-openai-agents-sdk",
                "17-claude-agent-sdk",
                "18-agno-and-mastra-runtimes",
                "19-benchmarks-swebench-gaia",
                "20-benchmarks-webarena-osworld",
                "21-computer-use-agents",
                "22-voice-agents-pipecat-livekit",
                "23-otel-genai-conventions",
                "24-agent-observability-platforms",
                "25-multi-agent-debate",
                "26-failure-modes-agentic",
                "27-prompt-injection-defense",
                "28-orchestration-patterns",
                "29-production-runtimes",
                "30-eval-driven-agent-development",
                "31-agent-workbench-why-models-fail",
                "32-minimal-agent-workbench",
                "33-instructions-as-executable-constraints",
                "34-repo-memory-and-state",
                "35-initialization-scripts",
                "36-scope-contracts",
                "37-runtime-feedback-loops",
                "38-verification-gates",
                "39-reviewer-agent",
                "40-multi-session-handoff",
                "41-workbench-for-real-repos",
                "42-agent-workbench-capstone",
              ],
            },
            {
              text: "第 15 阶段 · 自治系统",
              prefix: "15-autonomous-systems/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-long-horizon-agents",
                "02-star-family-reasoning",
                "03-alphaevolve-evolutionary-coding",
                "04-darwin-godel-machine",
                "05-ai-scientist-v2",
                "06-automated-alignment-research",
                "07-recursive-self-improvement",
                "08-bounded-self-improvement",
                "09-coding-agent-landscape",
                "10-claude-code-permission-modes",
                "11-browser-agents",
                "12-durable-execution",
                "13-cost-governors",
                "14-kill-switches-canaries",
                "15-propose-then-commit",
                "16-checkpoints-rollback",
                "17-constitutional-ai",
                "18-llama-guard",
                "19-anthropic-rsp",
                "20-openai-preparedness-deepmind-fsf",
                "21-metr-external-evaluation",
                "22-cais-caisi-societal-risk",
              ],
            },
            {
              text: "第 16 阶段 · 多智能体与群体",
              prefix: "16-multi-agent-and-swarms/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-why-multi-agent",
                "02-fipa-acl-heritage",
                "03-communication-protocols",
                "04-primitive-model",
                "05-supervisor-orchestrator-pattern",
                "06-hierarchical-architecture",
                "07-society-of-mind-debate",
                "08-role-specialization",
                "09-parallel-swarm-networks",
                "10-group-chat-speaker-selection",
                "11-handoffs-and-routines",
                "12-a2a-protocol",
                "13-shared-memory-blackboard",
                "14-consensus-and-bft",
                "15-voting-debate-topology",
                "16-negotiation-bargaining",
                "17-generative-agents-simulation",
                "18-theory-of-mind-coordination",
                "19-swarm-optimization-pso-aco",
                "20-marl-maddpg-qmix-mappo",
                "21-agent-economies",
                "22-production-scaling-queues-checkpoints",
                "23-failure-modes-mast-groupthink",
                "24-evaluation-coordination-benchmarks",
                "25-case-studies-2026-sota",
              ],
            },
            {
              text: "第 17 阶段 · 基础设施与生产",
              prefix: "17-infrastructure-and-production/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-managed-llm-platforms",
                "02-inference-platform-economics",
                "03-gpu-autoscaling-kubernetes",
                "04-vllm-serving-internals",
                "05-eagle3-speculative-decoding",
                "06-sglang-radixattention",
                "07-tensorrt-llm-blackwell",
                "08-inference-metrics-goodput",
                "09-production-quantization",
                "10-cold-start-mitigation",
                "11-multi-region-kv-locality",
                "12-edge-inference",
                "13-llm-observability",
                "14-prompt-semantic-caching",
                "15-batch-apis",
                "16-model-routing",
                "17-disaggregated-prefill-decode",
                "18-vllm-production-stack-lmcache",
                "19-ai-gateways",
                "20-shadow-canary-progressive",
                "21-ab-testing-llm-features",
                "22-load-testing-llm-apis",
                "23-sre-for-ai",
                "24-chaos-engineering-llm",
                "25-security-secrets-audit",
                "26-compliance-frameworks",
                "27-finops-llms",
                "28-self-hosted-serving-selection",
              ],
            },
            {
              text: "第 18 阶段 · 伦理·安全·对齐",
              prefix: "18-ethics-safety-alignment/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-instruction-following-alignment-signal",
                "02-reward-hacking-goodhart",
                "03-direct-preference-optimization-family",
                "04-sycophancy-rlhf-amplification",
                "05-constitutional-ai-rlaif",
                "06-mesa-optimization-deceptive-alignment",
                "07-sleeper-agents-persistent-deception",
                "08-in-context-scheming-frontier-models",
                "09-alignment-faking",
                "10-ai-control-subversion",
                "11-scalable-oversight-weak-to-strong",
                "12-red-teaming-pair-automated-attacks",
                "13-many-shot-jailbreaking",
                "14-ascii-art-visual-jailbreaks",
                "15-indirect-prompt-injection",
                "16-red-team-tooling-garak-llamaguard-pyrit",
                "17-wmdp-dual-use-evaluation",
                "18-frontier-safety-frameworks-rsp-pf-fsf",
                "19-model-welfare-research",
                "20-bias-representational-harm",
                "21-fairness-criteria-group-individual-counterfactual",
                "22-differential-privacy-for-llms",
                "23-watermarking-synthid-stable-signature-c2pa",
                "24-regulatory-frameworks-eu-us-uk-korea",
                "25-echoleak-cves-for-ai",
                "26-model-system-dataset-cards",
                "27-data-provenance-training-governance",
                "28-alignment-research-ecosystem",
                "29-moderation-systems-openai-perspective-llamaguard",
                "30-dual-use-risk-cyber-bio-chem-nuclear",
              ],
            },
            {
              text: "第 19 阶段 · 结课项目",
              prefix: "19-capstone-projects/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "01-terminal-native-coding-agent",
                "02-rag-over-codebase",
                "03-realtime-voice-assistant",
                "04-multimodal-document-qa",
                "05-autonomous-research-agent",
                "06-devops-troubleshooting-agent",
                "07-end-to-end-fine-tuning-pipeline",
                "08-production-rag-chatbot",
                "09-code-migration-agent",
                "10-multi-agent-software-team",
                "11-llm-observability-dashboard",
                "12-video-understanding-pipeline",
                "13-mcp-server-with-registry",
                "14-speculative-decoding-server",
                "15-constitutional-safety-harness",
                "16-github-issue-to-pr-agent",
                "17-personal-ai-tutor",
                "20-agent-harness-loop-contract",
                "21-tool-registry-schema-validation",
                "22-jsonrpc-stdio-transport",
                "23-function-call-dispatcher",
                "24-plan-execute-control-flow",
                "25-verification-gates-observation-budget",
                "26-sandbox-runner-denylist",
                "27-eval-harness-fixture-tasks",
                "28-observability-otel-traces",
                "29-end-to-end-coding-task-demo",
                "30-bpe-tokenizer-from-scratch",
                "31-tokenized-dataset-sliding-window",
                "32-token-positional-embeddings",
                "33-multihead-self-attention",
                "34-transformer-block",
                "35-gpt-model-assembly",
                "36-training-loop-eval",
                "37-loading-pretrained-weights",
                "38-classifier-finetuning",
                "39-instruction-tuning-sft",
                "40-dpo-from-scratch",
                "41-eval-pipeline",
                "42-large-corpus-downloader",
                "43-hdf5-tokenized-corpus",
                "44-cosine-lr-warmup",
                "45-gradient-clipping-amp",
                "46-gradient-accumulation",
                "47-checkpoint-save-resume",
                "48-distributed-fsdp-ddp",
                "49-lm-eval-harness",
                "50-hypothesis-generator",
                "51-literature-retrieval",
                "52-experiment-runner",
                "53-result-evaluator",
                "54-paper-writer",
                "55-critic-loop",
                "56-iteration-scheduler",
                "57-end-to-end-research-demo",
                "58-vision-encoder-patches",
                "59-vit-transformer",
                "60-projection-layer-modality-align",
                "61-cross-attention-fusion",
                "62-vision-language-pretraining",
                "63-multimodal-eval",
                "64-chunking-strategies-advanced",
                "65-hybrid-retrieval-bm25-dense",
                "66-reranker-cross-encoder",
                "67-query-rewriting-hyde",
                "68-rag-eval-precision-recall",
                "69-end-to-end-rag-system",
                "70-task-spec-format",
                "71-classical-metrics",
                "72-code-exec-metric",
                "73-perplexity-calibration",
                "74-leaderboard-aggregation",
                "75-end-to-end-eval-runner",
                "76-collective-ops-from-scratch",
                "77-data-parallel-ddp",
                "78-zero-parameter-sharding",
                "79-pipeline-parallel",
                "80-checkpoint-sharded-resume",
                "81-end-to-end-distributed-train",
                "82-jailbreak-taxonomy",
                "83-prompt-injection-detector",
                "84-refusal-evaluation",
                "85-content-classifier-integration",
                "86-constitutional-rules-engine",
                "87-end-to-end-safety-gate",
              ],
            },
          ],
        },
      ],
    },
    {
      text: "运维与部署",
      icon: "fa6-solid:rocket",
      prefix: "运维与部署/",
      collapsible: true,
      children: [
        "",
        {
          text: "安卓 APK 离线打包部署",
          icon: "fa6-brands:android",
          link: "安卓APK离线打包部署.html",
        },
        {
          text: "Docker & Kubernetes",
          icon: "fa6-brands:docker",
          prefix: "Docker_K8s/",
          collapsible: true,
          children: [
            "",
            {
              text: "01 · Docker 基础",
              icon: "fa6-brands:docker",
              prefix: "01_Docker基础/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "02 · Docker 进阶",
              icon: "fa6-solid:layer-group",
              prefix: "02_Docker进阶/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "03 · Docker 生产实战",
              icon: "fa6-solid:flask",
              prefix: "03_Docker生产实战/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "04 · Kubernetes 基础",
              icon: "fa6-solid:dharmachakra",
              prefix: "04_Kubernetes基础/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "05 · Kubernetes 进阶",
              icon: "fa6-solid:rocket",
              prefix: "05_Kubernetes进阶/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "06 · Kubernetes 运维",
              icon: "fa6-solid:wrench",
              prefix: "06_Kubernetes运维/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "07 · Kubernetes 生产实战",
              icon: "fa6-solid:building",
              prefix: "07_Kubernetes生产实战/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "08 · 云原生生态",
              icon: "fa6-solid:cloud",
              prefix: "08_云原生生态/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "09 · 面试题精选",
              icon: "fa6-solid:file-circle-question",
              prefix: "09_面试题精选/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
      ],
    },
    {
      text: "Linux",
      icon: "fa6-brands:linux",
      prefix: "Linux/",
      collapsible: true,
      children: [
        "",
        {
          text: "01 · Linux 基础",
          icon: "fa6-solid:cube",
          prefix: "01_Linux基础/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "02 · Linux 进阶",
          icon: "fa6-solid:layer-group",
          prefix: "02_Linux进阶/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "03 · Linux 高级",
          icon: "fa6-solid:star",
          prefix: "03_Linux高级/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "04 · 三剑客",
          icon: "fa6-solid:scissors",
          prefix: "04_三剑客/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "05 · 生产级实战",
          icon: "fa6-solid:flask",
          prefix: "05_生产级实战/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "06 · 内核级",
          icon: "fa6-solid:microchip",
          prefix: "06_内核级/",
          collapsible: true,
          children: "structure",
        },
        {
          text: "07 · Nginx",
          icon: "fa6-solid:server",
          prefix: "07_Nginx/",
          collapsible: true,
          children: [
            "",
            {
              text: "01 · Nginx 基础与架构",
              icon: "fa6-solid:cube",
              prefix: "01_Nginx基础与架构/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "02 · 核心配置指令",
              icon: "fa6-solid:sliders",
              prefix: "02_核心配置指令/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "03 · HTTP 核心与虚拟主机",
              icon: "fa6-solid:globe",
              prefix: "03_HTTP核心与虚拟主机/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "04 · 反向代理与负载均衡",
              icon: "fa6-solid:arrows-split-up-and-left",
              prefix: "04_反向代理与负载均衡/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "05 · SSL/TLS 与 HTTPS",
              icon: "fa6-solid:lock",
              prefix: "05_SSL_TLS与HTTPS/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "06 · 限流与访问控制",
              icon: "fa6-solid:shield-halved",
              prefix: "06_限流与访问控制/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "07 · 缓存与性能优化",
              icon: "fa6-solid:gauge-high",
              prefix: "07_缓存与性能优化/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "08 · 高级特性与扩展",
              icon: "fa6-solid:puzzle-piece",
              prefix: "08_高级特性与扩展/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "09 · 生产级实战",
              icon: "fa6-solid:flask",
              prefix: "09_生产级实战/",
              collapsible: true,
              children: "structure",
            },
          ],
        },
      ],
    },
    {
      text: "面试题",
      icon: "fa6-solid:file-circle-question",
      prefix: "面试题/",
      collapsible: true,
      children: "structure",
    },
    {
      text: "业务系统",
      icon: "fa6-solid:building",
      prefix: "业务系统/",
      collapsible: true,
      children: "structure",
    },
    {
      text: "语言",
      icon: "fa6-solid:language",
      prefix: "语言/",
      collapsible: true,
      children: [
        {
          text: "英语",
          icon: "fa6-solid:language",
          prefix: "英语/",
          collapsible: true,
          children: [
            {
              text: "语法俱乐部",
              icon: "fa6-solid:book",
              prefix: "语法俱乐部/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "音标有谱",
              icon: "fa6-solid:music",
              prefix: "音标/",
              collapsible: true,
              children: "structure",
            },
            {
              text: "Friends 老友记",
              icon: "fa6-solid:tv",
              prefix: "Friends老友记/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "",
                { text: "第一季", icon: "fa6-solid:tv", prefix: "S01/", collapsible: true, sidebarSorter: ["readme", "order", "filename"], children: "structure" },
                { text: "第二季", icon: "fa6-solid:tv", prefix: "S02/", collapsible: true, sidebarSorter: ["readme", "order", "filename"], children: "structure" },
                { text: "第三季", icon: "fa6-solid:tv", prefix: "S03/", collapsible: true, sidebarSorter: ["readme", "order", "filename"], children: "structure" },
                { text: "第四季", icon: "fa6-solid:tv", prefix: "S04/", collapsible: true, sidebarSorter: ["readme", "order", "filename"], children: "structure" },
                { text: "第五季", icon: "fa6-solid:tv", prefix: "S05/", collapsible: true, sidebarSorter: ["readme", "order", "filename"], children: "structure" },
                { text: "第六季", icon: "fa6-solid:tv", prefix: "S06/", collapsible: true, sidebarSorter: ["readme", "order", "filename"], children: "structure" },
                { text: "第七季", icon: "fa6-solid:tv", prefix: "S07/", collapsible: true, sidebarSorter: ["readme", "order", "filename"], children: "structure" },
                { text: "第八季", icon: "fa6-solid:tv", prefix: "S08/", collapsible: true, sidebarSorter: ["readme", "order", "filename"], children: "structure" },
                { text: "第九季", icon: "fa6-solid:tv", prefix: "S09/", collapsible: true, sidebarSorter: ["readme", "order", "filename"], children: "structure" },
                { text: "第十季", icon: "fa6-solid:tv", prefix: "S10/", collapsible: true, sidebarSorter: ["readme", "order", "filename"], children: "structure" },
              ],
            },
          ],
        },
        {
          text: "日语",
          icon: "fa6-solid:book-open",
          prefix: "日语/",
          collapsible: true,
          sidebarSorter: ["readme", "order", "filename"],
          children: [
            "",
            { text: "五十音图", icon: "fa6-solid:language", link: "五十音图" },
            { text: "新标日初级(二)单词", icon: "fa6-solid:book", link: "00.新标日初级(二)单词" },
            { text: "单词速记表格", icon: "fa6-solid:table", link: "单词速记" },
            {
              text: "课本",
              icon: "fa6-solid:book-open",
              prefix: "课本/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                {
                  text: "简明日语NHK（2015）",
                  icon: "fa6-solid:headphones",
                  prefix: "简明日语NHK/",
                  collapsible: true,
                  sidebarSorter: ["readme", "order", "filename"],
                  children: "structure",
                },
                {
                  text: "简明日语（2019）",
                  icon: "fa6-solid:book",
                  prefix: "简明日语/",
                  collapsible: true,
                  sidebarSorter: ["readme", "order", "filename"],
                  children: "structure",
                },
              ],
            },
            {
              text: "语法",
              icon: "fa6-solid:book",
              prefix: "语法/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "",
                { text: "导论", icon: "fa6-solid:book", link: "01-导论" },
                { text: "书写系统", icon: "fa6-solid:book", link: "02-书写系统" },
                { text: "基础语法", icon: "fa6-solid:book", link: "03-基础语法" },
                { text: "核心语法", icon: "fa6-solid:book", link: "04-核心语法" },
                { text: "特殊表达", icon: "fa6-solid:book", link: "05-特殊表达" },
                { text: "高级话题", icon: "fa6-solid:book", link: "06-高级话题" },
                { text: "基本句型与语序", icon: "fa6-solid:book", link: "基本句型" },
                { text: "名词分类与数词表达", icon: "fa6-solid:book", link: "名词与数词" },
                { text: "动词变形总览", icon: "fa6-solid:book", link: "动词变形" },
                { text: "助词详解", icon: "fa6-solid:book", link: "助词详解" },
                { text: "形容词变形与活用", icon: "fa6-solid:book", link: "形容词变形" },
                { text: "敬语体系", icon: "fa6-solid:book", link: "敬语体系" },
                { text: "复合动词与补助动词", icon: "fa6-solid:book", link: "复合动词" },
                { text: "条件假定表达", icon: "fa6-solid:book", link: "条件假定" },
              ],
            },
            {
              text: "阅读",
              icon: "fa6-solid:book-open-reader",
              prefix: "阅读/",
              collapsible: true,
              sidebarSorter: ["readme", "order", "filename"],
              children: [
                "",
                { text: "桃太郎", icon: "fa6-solid:book", link: "01-桃太郎" },
                { text: "浦島太郎", icon: "fa6-solid:book", link: "02-浦島太郎" },
                { text: "笠地蔵", icon: "fa6-solid:book", link: "03-笠地蔵" },
                { text: "鶴の恩返し", icon: "fa6-solid:book", link: "04-鶴の恩返し" },
                { text: "一寸法師", icon: "fa6-solid:book", link: "05-一寸法師" },
                { text: "花咲か爺", icon: "fa6-solid:book", link: "06-花咲か爺" },
                { text: "猿蟹合戦", icon: "fa6-solid:book", link: "07-猿蟹合戦" },
                { text: "ねずみの嫁入り", icon: "fa6-solid:book", link: "08-ねずみの嫁入り" },
                { text: "竹取物語", icon: "fa6-solid:book", link: "09-竹取物語" },
                { text: "一休さん", icon: "fa6-solid:book", link: "10-一休さん" },
                { text: "桜の開花", icon: "fa6-solid:newspaper", link: "11-桜の開花" },
                { text: "コンビニ", icon: "fa6-solid:newspaper", link: "12-コンビニ" },
                { text: "ロボット", icon: "fa6-solid:newspaper", link: "13-ロボット" },
                { text: "初詣", icon: "fa6-solid:newspaper", link: "14-初詣" },
                { text: "黄金週間", icon: "fa6-solid:newspaper", link: "15-黄金週間" },
                { text: "お盆", icon: "fa6-solid:newspaper", link: "16-お盆" },
                { text: "紅葉", icon: "fa6-solid:newspaper", link: "17-紅葉" },
                { text: "雪まつり", icon: "fa6-solid:newspaper", link: "18-雪まつり" },
                { text: "敬老の日", icon: "fa6-solid:newspaper", link: "19-敬老の日" },
                { text: "かちかちやま", icon: "fa6-solid:book", link: "20-かちかちやま" },
                { text: "おむすびころりん", icon: "fa6-solid:book", link: "21-おむすびころりん" },
                { text: "わらしべ長者", icon: "fa6-solid:book", link: "22-わらしべ長者" },
                { text: "したきりすずめ", icon: "fa6-solid:book", link: "23-したきりすずめ" },
                { text: "きんたろう", icon: "fa6-solid:book", link: "24-きんたろう" },
                { text: "こぶとりじいさん", icon: "fa6-solid:book", link: "25-こぶとりじいさん" },
                { text: "いなばのしろウサギ", icon: "fa6-solid:book", link: "26-いなばのしろウサギ" },
                { text: "ぶんぶくちゃがま", icon: "fa6-solid:book", link: "27-ぶんぶくちゃがま" },
                { text: "ゆきおんな", icon: "fa6-solid:book", link: "28-ゆきおんな" },
                { text: "しょうじょうじのたぬきばやし", icon: "fa6-solid:book", link: "29-しょうじょうじのたぬきばやし" },
              ],
            },
          ],
        },
      ],
    },
  ],
});
