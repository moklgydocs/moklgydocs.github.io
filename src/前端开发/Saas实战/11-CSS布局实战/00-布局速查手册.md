...
│          padding               │
└─────────────────────────────────┘
→ width: 100px + padding: 20px + border: 1px = 实际占 142px

border-box（Tailwind 默认）：
┌─────────────────────────────────┐
│          padding               │
│   ┌───────────────────────┐    │
│   │       content         │    │  宽高 = content + padding + border
│   └───────────────────────┘    │
│          padding               │
└─────────────────────────────────┘
→ width: 100px 包含一切，实际占 100px
```
...
0px          relative（正常位置）
  ↓          跟随滚动
top: 56px    → 到达阈值，切换为 fixed
  ↓          固定在 top: 56px 的位置
继续滚动      一直固定，直到父容器底部
父容器底部    消失（sticky 不能超出父容器）
```
...
手机浏览器（有地址栏）：
  100vh = 最大视口高度（包含地址栏区域）✗
  100dvh = 当前可见视口高度 ✓

当用户向下滚动，地址栏收缩：
  100vh 不变（仍然是最大值）
  100dvh 变大（地址栏收缩后可见区域变大）
```
