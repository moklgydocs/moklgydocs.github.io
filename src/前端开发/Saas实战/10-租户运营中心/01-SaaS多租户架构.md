...
- ASP.NET Core 的 `IHttpClientFactory` 命名客户端模式
- EF Core 的 `HasQueryFilter` 行级租户隔离
- React Router 路由配置与 `lazy` 懒加载
- Zustand Store 的全局状态读取（`useAuthStore.getState()`）

---

## 前置知识

- 了解 ASP.NET Core EF Core 的 `HasQueryFilter` 全局查询过滤机制
- 理解数据库隔离级别的基本概念（独立数据库、共享数据库+行隔离、Schema 隔离）
- 熟悉 Vite proxy 的路径前缀转发机制
...
> 🤔 **导师提问**：独立数据库策略意味着每个租户有独立的连接串。运营中心要查看"所有租户的活跃用户数"，需要连接每个租户的数据库分别查询——这比共享数据库+行隔离的 `SELECT COUNT(*) FROM Users` 慢得多。这种跨租户聚合查询在独立数据库策略下应该怎么优化？提示：考虑汇总表、事件驱动同步或 CQRS 读模型。

🤔 **导师提问**：如果我们采用共享数据库+行隔离策略，前端每个 API 请求都需要在请求头或参数中带上 `tenantId`。那运营中心的 API 调用呢？运营中心看到的是全局数据，不需要 `tenantId`——但如果后端全局启用 `HasQueryFilter`，运营中心怎么绕过？
...
🤔 **导师提问**：在 ASP.NET Core 中，我们通常用 Middleware 注入 `TenantId`。那运营中心的 Controller 怎么"跳过"这个 Middleware？常见方案：在 Middleware 中判断请求路径前缀，`/ops-api` 开头的跳过租户注入。前端 Vite Proxy 的路径前缀正好对应了后端的这个分支。

> 🤔 **导师提问**：如果后端全局启用了 `HasQueryFilter(b => b.TenantId == tenantId)` 做租户隔离，运营中心的 Controller 怎么"绕过"这个 Filter？提示：常见方案有三种——1）用 `IgnoreQueryFilters()` 逐查询禁用；2）在 Middleware 中判断路径前缀，`/ops-api` 不注入 TenantId；3）运营中心用独立的 DbContext（不带 Filter）。你倾向于哪种？
...
🤔 **导师提问**：`featuresJson` 为什么用 `string | null` 而不是 `Record<string, unknown>`？因为后端数据库中 JSON 字段存的是字符串，前端可以 `JSON.parse(featuresJson)` 按需解析。这和 EF Core 中 `HasColumnType("jsonb")` 的做法一样——数据库存 JSON 字符串，代码中按需反序列化。
...
🤔 **导师提问**：如果后端的 Provisioning 过程可能失败（比如数据库创建超时），租户会永远停留在 Provisioning 状态吗？我们需要一个超时机制或手动重试按钮吗？
...
🤔 **导师提问**：为什么路径是 `/api/tenant-ops/...` 而不是 `/api/ops/...`？`tenant-ops` 明确了这是"租户运营"操作，和未来可能出现的"用户运营"、"内容运营"区分开。前端的 `createHttp("/ops-api")` 只是 Vite Proxy 的前缀，真正的业务路径在后端定义。
...
🤔 **导师提问**：租户详情页为什么不用 URL 的 query 参数来切换标签（如 `?tab=quota`），而是用组件内部状态？因为标签页切换只是 UI 状态变化，不需要被浏览器历史记录或书签捕获。但如果需要"直接链接到某个标签页"的功能，就需要 URL 参数了。
...
🤔 **导师提问**：如果你的 SaaS 产品面向个人用户（如笔记应用），租户数量可能达到百万级。独立数据库策略还适用吗？这时候共享数据库+行隔离可能更现实——但你需要特别关注 `TenantId` 过滤的正确性和查询性能。
...
1. **平台级数据 vs 租户级数据的混淆**：即使是运营中心的"全局视角"，查看租户详情时仍然需要传 `tenantId`。这不是租户过滤，而是**指定操作对象**——就像 DBA 执行 `USE TenantDB_A` 一样，是在选择目标，不是在限制权限。
2. **租户 ID 跨请求泄漏**：在运营中心页面，我们同时操作多个租户。如果在一个租户的上下文中发起请求，却把 `tenantId` 残留在下一个请求中，就会造成数据错乱。运营中心的 API 都显式传 `tenantId`，而不是依赖全局状态或请求头。
3. **Terminated 是终态不可逆**：终止租户就像数据库被 `DROP` 了，不是 `OFFLINE`。误操作无法恢复，所以前端必须有二次确认。
4. **Provisioning 是过渡状态**：租户创建时后端需要异步初始化数据库、分配资源。前端调用 `provision` API 后，租户进入 `Provisioning` 状态，后端完成后自动变为 `Active`。前端不需要轮询。
...
### 概念级（理解 What）
1. 运营中心的目标用户是谁？为什么它需要看到所有租户的数据，而不是只看自己租户的？

### 推理级（推导 What-if）
2. 如果租户从 Suspended 恢复到 Active，但它的版本订阅已过期，会发生什么？前端应该怎么提示？后端应该怎么处理？

### 动手级（设计 Why）
3. 假设我们要新增一个"归档(Archived)"状态，它和 Terminated 有什么区别？需要修改哪些 API？归档的租户数据库是否还保留？成本如何？
