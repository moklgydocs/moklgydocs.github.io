...
graph TD
    ...
    B -->|"Func<T,bool>"| C["编译为IL委托<br/>可直接调用<br/>func(product)"]
    B -->|"Expression<Func<T,bool>>"| D["编译为表达式树<br/>数据结构<br/>可被解析/翻译"]
    D --> E["EF Core解析表达式树"]
    E --> F["翻译为SQL语句"]
    F --> G["发送到数据库执行"]
...
```mermaid
graph TD
    A["Lambda<br/>p => p.Price > 100"] --> B["BinaryExpression<br/>GreaterThan"]
    B --> C["MemberAccess<br/>p.Price"]
    B --> D["Constant<br/>100"]
    C --> E["Parameter<br/>p"]
...
```mermaid
graph TD
    A["IQueryable<br/>表达式树管线"] -->|"Where(p => p.Price > 100)"| A
    A -->|"Select(p => p.Name)"| A
    A -->|"ToList()"| B["SQL执行<br/>数据库返回结果"]

    C["IEnumerable<br/>委托管线"] -->|"Where(Func)"| C
    C -->|"Select(Func)"| C
    C -->|"ToList()"| D["内存执行<br/>逐条遍历"]
...
```mermaid
graph LR
    A["LINQ查询<br/>Where/Select/..."] --> B["表达式树构建"]
    B --> C["IQueryProvider.Execute"]
    C --> D["表达式树遍历/解析"]
    D --> E["翻译为目标语言<br/>SQL/Cypher/..."]
    E --> F["发送到数据源执行"]
    F --> G["结果映射回对象"]
```
