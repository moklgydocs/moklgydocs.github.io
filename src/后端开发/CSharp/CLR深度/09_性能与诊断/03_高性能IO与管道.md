...
```csharp
// 使用 ArrayPool + 自定义 MemoryOwner 包装
using var owner = new PooledMemoryOwner<byte>(ArrayPool<byte>.Shared, 1024);
...
// 注意：ArrayPool.Shared.Rent() 返回 T[]，不是 IMemoryOwner<T>
// 需要像 3.3 节那样自行包装才能实现 using 模式
```
...
// io_uring 在支持的系统上自动启用
// 可通过环境变量禁用:
// DOTNET_System_IO_IoUring=0
...
internal class BufferSegment
{
    ...
    public Memory<byte> CommittedMemory =>
