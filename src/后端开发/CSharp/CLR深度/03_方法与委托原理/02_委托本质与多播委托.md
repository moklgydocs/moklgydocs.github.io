...
IL_0000:  ldloc.0
IL_0001:  stloc.1
// 协变赋值是 CLR 类型系统的隐式引用转换
// Func\<string\> IS-A Func\<object\>，无需 CreateDelegate 调用
```
...
public class DelegateBenchmark
{
    ...
    private readonly Func<Calculator, int, int, int> _openDel;  // 开放委托

    public DelegateBenchmark()
    {
        _del = _calc.Add;
        // 开放实例委托：第一个参数为 target，无需在构造时指定
        _openDel = (Func<Calculator, int, int, int>)Delegate.CreateDelegate(
            typeof(Func<Calculator, int, int, int>), null,
...
// 静态方法：函数指针签名无需 this 参数
public static int StaticAdd(int x, int y) => x + y;
delegate* managed<int, int, int> staticPtr = &StaticAdd;
int result1 = staticPtr(1, 2);  // 直接调用，无委托开销

// 实例方法：函数指针签名必须包含 this 类型作为第一个参数
public int Add(int x, int y) => x + y;
delegate* managed<Calculator, int, int, int> instancePtr = &Add;
int result2 = instancePtr(_calc, 1, 2);  // 显式传入 this
```
