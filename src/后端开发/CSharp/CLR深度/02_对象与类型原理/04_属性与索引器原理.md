...
`init` 访问器在 IL 层面并不是一个简单的方法修饰符。CLR 本身没有 `init` 概念，C# 编译器通过 `initonly` modreq（修饰需求）来实现编译时限制：
...
public class DerivedPerson : Person
{
    ...
    {
        // Name = "Test";  // CS8852: 派生类构造函数中也不能赋值
    }
}
```
...
- 但 `init` 属性在派生类构造函数中**不能**赋值（这与 `readonly` 不同）
