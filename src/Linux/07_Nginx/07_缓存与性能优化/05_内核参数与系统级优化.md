...
::: warning tcp_fin_timeout 不控制 TIME_WAIT
`tcp_fin_timeout` 控制的是 **FIN-WAIT-2** 状态的超时时间，而非 TIME_WAIT。TIME_WAIT 的持续时间在 Linux 内核中硬编码为 2×MSL（60秒），无法通过 sysctl 修改。很多文章将二者混淆，这是常见误区。
:::

```bash
# FIN-WAIT-2 状态的超时时间（秒）
# 当本端已关闭连接（发送FIN并收到ACK），但对端尚未关闭时，
# 本端连接处于 FIN-WAIT-2 状态，此参数控制该状态的最长持续时间
# 默认 60 秒
# 降低此值可以更快回收 FIN-WAIT-2 连接（对端异常不关闭时）
net.ipv4.tcp_fin_timeout = 30

# 注意：此参数不影响 TIME_WAIT 状态
# TIME_WAIT 持续时间固定为 2×MSL（60秒），内核硬编码，不可修改
# 减少 TIME_WAIT 数量的正确方法是：
#   1. 开启 tcp_tw_reuse（允许将 TIME_WAIT 连接用于新连接）
#   2. 增加 tcp_max_tw_buckets（增大允许的 TIME_WAIT 数量上限）
#   3. 使用长连接（keepalive）减少短连接产生的 TIME_WAIT
# 推荐范围：15-60 秒
# 局域网环境可以设为 15-30 秒
# 公网环境建议保持 30-60 秒
```
...
# FACK 已在 Linux 4.7+ 中移除，功能合并到 SACK
# 在现代内核（4.7+）中设置此参数会报错，可忽略
# net.ipv4.tcp_fack = 1
...
net.ipv4.tcp_fin_timeout = 30    # 缩短 FIN-WAIT-2 时间（不影响 TIME_WAIT）
...
net.ipv4.tcp_timestamps = 1
net.ipv4.tcp_sack = 1
# net.ipv4.tcp_fack = 1  # [已废弃] Linux 4.7+ 中 FACK 合并到 SACK
...
net.ipv4.tcp_retries2 = 5  # 默认15；5适用于数据中心/低延迟环境；公网建议8-10
```
