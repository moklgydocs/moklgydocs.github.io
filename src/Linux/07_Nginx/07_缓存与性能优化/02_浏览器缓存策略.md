...
# If-None-Match: "65a1b2c0-1f4a"
# If-Modified-Since: Wed, 03 Jan 2024 10:30:00 GMT
# Nginx 优先验证 ETag，ETag 通过后再验证 Last-Modified
# 两者都通过才返回 304
```

::: important ETag 优先级更高
当请求同时包含 `If-None-Match` 和 `If-Modified-Since` 时，Nginx 只检查 `If-None-Match`。只有 `If-None-Match` 不存在时，才会检查 `If-Modified-Since`。这是 RFC 7232 的规定。
:::
